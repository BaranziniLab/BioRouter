...
#   BIOROUTER_BIN_DIR  - Directory to which biorouter will be installed (default: $HOME/.local/bin)
#   BIOROUTER_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Overrides CANARY. Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   BIOROUTER_PROVIDER - Optional: provider for biorouter
#   BIOROUTER_MODEL    - Optional: model for biorouter
#   CANARY         - Optional: if set to "true", downloads from canary release instead of stable
#   CONFIGURE      - Optional: if set to "false", disables running biorouter configure interactively
...
if ! command -v curl >/dev/null 2>&1; then
  echo "Error: 'curl' is required to download biorouter. Please install curl and try again."
  exit 1
fi

# Check for tar or unzip (depending on OS)
if ! command -v tar >/dev/null 2>&1 && ! command -v unzip >/dev/null 2>&1; then
  echo "Error: Either 'tar' or 'unzip' is required to extract biorouter. Please install one and try again."
  exit 1
fi
...
if [[ "${WINDIR:-}" ]] || [[ "${windir:-}" ]] || [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    ...
    DEFAULT_BIN_DIR="$USERPROFILE/biorouter"
...
BIOROUTER_BIN_DIR="${BIOROUTER_BIN_DIR:-$DEFAULT_BIN_DIR}"
RELEASE="${CANARY:-false}"
CONFIGURE="${CONFIGURE:-true}"
if [ -n "${BIOROUTER_VERSION:-}" ]; then
  # Validate the version format
  if [[ ! "$BIOROUTER_VERSION" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$ ]]; then
    echo "[error]: invalid version '$BIOROUTER_VERSION'."
    echo "  expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
    exit 1
  fi
  BIOROUTER_VERSION=$(echo "$BIOROUTER_VERSION" | sed 's/^v\{0,1\}/v/') # Ensure the version string is prefixed with 'v' if not already present
  RELEASE_TAG="$BIOROUTER_VERSION"
else
  # If BIOROUTER_VERSION is not set, fall back to existing behavior for backwards compatibility
...
case "$OS" in
  ...
  mingw*|msys*|cygwin*)
    ...
    ;;
  *)
    echo "Error: Unsupported OS '$OS'. biorouter currently supports Linux, macOS, and Windows."
    exit 1
    ;;
esac
...
if [ "$OS" = "darwin" ]; then
  FILE="biorouter-$ARCH-apple-darwin.tar.bz2"
  EXTRACT_CMD="tar"
  ...
  FILE="biorouter-$ARCH-unknown-linux-gnu.tar.bz2"
  EXTRACT_CMD="tar"
fi

DOWNLOAD_URL="https://github.com/$REPO/releases/download/$RELEASE_TAG/$FILE"

# --- 4) Download & extract 'biorouter' binary ---
...
TMP_DIR="/tmp/biorouter_install_$RANDOM"
...
# Windows releases may contain files in a 'biorouter-package' subdirectory
EXTRACT_DIR="$TMP_DIR"
if [ "$OS" = "windows" ] && [ -d "$TMP_DIR/biorouter-package" ]; then
  echo "Found biorouter-package subdirectory, using that as extraction directory"
  EXTRACT_DIR="$TMP_DIR/biorouter-package"
fi

# Make binary executable
if [ "$OS" = "windows" ]; then
  chmod +x "$EXTRACT_DIR/biorouter.exe"
else
  chmod +x "$EXTRACT_DIR/biorouter"
fi

# --- 5) Install to $BIOROUTER_BIN_DIR ---
if [ ! -d "$BIOROUTER_BIN_DIR" ]; then
  echo "Creating directory: $BIOROUTER_BIN_DIR"
  mkdir -p "$BIOROUTER_BIN_DIR"
fi

echo "Moving biorouter to $BIOROUTER_BIN_DIR/$OUT_FILE"
if [ "$OS" = "windows" ]; then
  mv "$EXTRACT_DIR/biorouter.exe" "$BIOROUTER_BIN_DIR/$OUT_FILE"
else
  mv "$EXTRACT_DIR/biorouter" "$BIOROUTER_BIN_DIR/$OUT_FILE"
fi
...
if [ "$CONFIGURE" = true ]; then
  # --- 6) Configure biorouter (Optional) ---
  echo ""
  echo "Configuring biorouter"
  echo ""
  "$BIOROUTER_BIN_DIR/$OUT_FILE" configure
else
  echo "Skipping 'biorouter configure', you may need to run this manually later"
fi



# --- 7) Check PATH and give instructions if needed ---
if [[ ":$PATH:" != *":$BIOROUTER_BIN_DIR:"* ]]; then
  echo ""
  echo "Warning: biorouter installed, but $BIOROUTER_BIN_DIR is not in your PATH."

  if [ "$OS" = "windows" ]; then
    echo "To add biorouter to your PATH in PowerShell:"
    echo ""
    ...
    echo ""
    echo "The \$BIOROUTER_BIN_DIR is not in your PATH."
    ...
    if [ "$CONFIGURE" = true ]; then
      ...
      1)
        ...
        echo "Adding \$BIOROUTER_BIN_DIR to $RC_FILE..."
        echo "export PATH=\"$BIOROUTER_BIN_DIR:\$PATH\"" >> "$RC_FILE"
      ...
      2)
        ...
        echo "    export PATH=\"$BIOROUTER_BIN_DIR:\$PATH\""
        echo "Then reload your shell (e.g. 'source ~/.${SHELL_NAME}rc') to apply changes."
        ;;
      *)
        echo "Invalid choice. Please add \$BIOROUTER_BIN_DIR to your PATH manually."
        ;;
      esac
    else
      echo ""
      echo "Configure disabled. Please add \$BIOROUTER_BIN_DIR to your PATH manually."
    fi
