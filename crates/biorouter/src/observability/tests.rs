...
fn failed_tool_marked_error_and_matched_by_id() {
    ...
    assert!(
        matches!(shell.status, SpanStatus::Ok),
        "shell (id a) succeeded"
    );
    assert!(
        matches!(fetch.status, SpanStatus::Error),
        "fetch (id b) failed"
    );
...
fn subagent_and_compaction_nest_and_carry_attrs() {
    ...
    b.record(
        ObsEvent::SubAgentEnd {
            name: "researcher".into(),
        },
        8,
    );
    b.record(
        ObsEvent::CompactionStart {
            trigger: "auto".into(),
        },
        9,
    );
...
fn redaction_drops_free_form_attrs_but_keeps_token_counts() {
    ...
    assert!(
        redacted.spans()[0].attrs.is_none(),
        "redacted guardrail has no attrs"
    );
    ...
    assert!(
        open.spans()[0].attrs.is_some(),
        "unredacted guardrail keeps attrs"
    );
}
