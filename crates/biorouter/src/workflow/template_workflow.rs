...
use crate::workflow::{Workflow, BUILT_IN_WORKFLOW_DIR_PARAM};
use anyhow::Result;
...
pub fn render_workflow_content_with_params(
    content: &str,
    ...
    let env = add_template_in_env(
        ...
        params.get(BUILT_IN_WORKFLOW_DIR_PARAM).cloned(),
        UndefinedBehavior::Strict,
    )?;
    let template = env.get_template(CURRENT_TEMPLATE_NAME).unwrap();
    let rendered_content = template
        .render(params)
        .map_err(|e| anyhow::anyhow!("Failed to render the workflow {}", e))?;
    Ok(rendered_content)
}
...
fn add_template_in_env(
    ...
    if let Some(workflow_dir) = workflow_dir {
        env.set_loader(move |name| {
            let path = Path::new(workflow_dir.as_str()).join(name);
...
fn get_env_with_template_variables(
    ...
    let env = add_template_in_env(content, workflow_dir, undefined_behavior)?;
    let template = env.get_template(CURRENT_TEMPLATE_NAME).unwrap();
...
pub fn parse_workflow_content(
    content: &str,
    workflow_dir: Option<String>,
) -> Result<(Workflow, HashSet<String>)> {
    ...
    let workflow_content = if uses_template_inheritance(&preprocessed_content) {
        // Must render to resolve inheritance
        template
            .render(())
            .map_err(|e| anyhow::anyhow!("Failed to parse the workflow {}", e))?
    } else {
        // Preserve conditionals and variables as-is
        preprocessed_content
    };

    let workflow = Workflow::from_content(&workflow_content)?;
    // return workflow (without loading any variables) and the variable names that are in the workflow
    Ok((workflow, template_variables))
}
    ...
    mod render_content_with_params_tests {
        ...
        fn test_render_content_with_params() {
            ...
            let params = HashMap::from([
                ("workflow_dir".to_string(), "some_dir".to_string()),
                ("name".to_string(), "World".to_string()),
            ]);
            let result = render_workflow_content_with_params(content, &params).unwrap();
            ...
            let params = HashMap::from([("workflow_dir".to_string(), "some_dir".to_string())]);
            let err = render_workflow_content_with_params(content, &params).unwrap_err();
            let error_msg = err.to_string();
            assert!(error_msg.contains("Failed to render the workflow"));
...
"#;
            let params = HashMap::from([("workflow_dir".to_string(), "test_dir".to_string())]);
            let result = render_workflow_content_with_params(content, &params).unwrap();

            assert!(result.contains("prompt: ''"));
            assert!(!result.contains(r#"prompt: "\"\"""#)); // Should not contain escaped quotes

            assert!(result.contains(r#"name: "Simple Workflow""#));
        }
    }
}
