...
#[derive(Deserialize)]
#[serde(tag = "type")]
enum WorkflowExtensionConfigInternal {
    #[serde(rename = "stdio")]
    Stdio {
        name: String,
        #[serde(default)]
...
macro_rules! map_workflow_extensions {
    ($value:expr; $( $variant:ident { $( $field:ident ),* $(,)? } ),+ $(,)?) => {{
        match $value {
            $(
                WorkflowExtensionConfigInternal::$variant {
                    name,
                    description,
                    $( $field ),*
...
impl From<WorkflowExtensionConfigInternal> for ExtensionConfig {
    fn from(internal_variant: WorkflowExtensionConfigInternal) -> Self {
        map_workflow_extensions!(
        internal_variant;
            Stdio {
                cmd,
                args,
...
pub fn deserialize_workflow_extensions<'de, D>(
    deserializer: D,
) -> Result<Option<Vec<ExtensionConfig>>, D::Error>
where
    D: Deserializer<'de>,
{
    let remotes = Option::<Vec<WorkflowExtensionConfigInternal>>::deserialize(deserializer)?;
...
mod tests {
    ...
    #[derive(Deserialize)]
    struct Wrapper {
        #[serde(deserialize_with = "deserialize_workflow_extensions")]
