        ...
        match (client_cert_path, client_key_path) {
            ...
            (Some(_), None) => {
                return Err(anyhow::anyhow!(
                    "Client certificate provided (BIOROUTER_CLIENT_CERT_PATH) but no private key (BIOROUTER_CLIENT_KEY_PATH)"
                ));
            }
            (None, Some(_)) => {
                return Err(anyhow::anyhow!(
                    "Client private key provided (BIOROUTER_CLIENT_KEY_PATH) but no certificate (BIOROUTER_CLIENT_CERT_PATH)"
                ));
            }
            (None, None) => {}
        }

        if let Ok(ca_cert_path) = config.get_param::<String>("BIOROUTER_CA_CERT_PATH") {
