...
impl SessionManager {
    ...
    pub async fn diverge_session(
        ...
        self.storage
            .diverge_session(self, session_id, custom_name)
            .await
    }
...
impl SessionStorage {
    ...
    async fn diverge_session(
        ...
        // Lock the computed/custom name (so the auto-namer never clobbers the
        // branch marker) and record the lineage pointer.
        session_manager
            .update(&new_session.id)
...
mod tests {
    ...
    async fn test_diverge_session_names_branches_and_sets_lineage() {
        ...
        let b1 = sm.diverge_session(&original.id, None).await.unwrap();
        let b2 = sm.diverge_session(&original.id, None).await.unwrap();
    ...
    async fn test_diverge_of_a_branch_flattens_numbering() {
        ...
        let b2 = sm.diverge_session(&b1.id, None).await.unwrap();
    ...
    async fn test_diverge_placeholder_name_derives_from_conversation() {
        ...
        let branch = sm.diverge_session(&session.id, None).await.unwrap();
    ...
    async fn test_diverge_custom_name_overrides() {
        ...
        let original = seed_session_with_messages(&sm, 1).await;
        let branch = sm
            .diverge_session(&original.id, Some("  Hand Picked  ".to_string()))
            .await
            .unwrap();
