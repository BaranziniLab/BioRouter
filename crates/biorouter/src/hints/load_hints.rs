...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global biorouter hints.\n");
...
mod tests {
    ...
    fn test_goosehints_multiple_filenames() {
        ...
        fs::write(
            ...
            "Custom hints file content from .biorouterhints",
        )
        .unwrap();
        ...
        assert!(hints.contains("Custom hints file content from .biorouterhints"));
    }
    ...
    fn test_goosehints_configurable_filename() {
        ...
        assert!(!hints.contains(".biorouterhints")); // Make sure it's not loading the default
    }
    ...
    fn test_nested_goosehints_with_git_root() {
        ...
        fs::write(
            current_dir.join(BIOROUTER_HINTS_FILENAME),
    ...
    fn test_nested_goosehints_mixed_filenames() {
        ...
        assert!(hints.contains("Subdir .biorouterhints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(BIOROUTER_HINTS_FILENAME), hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(BIOROUTER_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(BIOROUTER_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(BIOROUTER_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[BIOROUTER_HINTS_FILENAME.to_string()], &gitignore);
