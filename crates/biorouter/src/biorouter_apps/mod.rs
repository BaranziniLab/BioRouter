...
impl McpAppCache {
    ...
    pub fn list_apps(&self) -> Result<Vec<BioRouterApp>, std::io::Error> {
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match fs::read_to_string(&path) {
                    Ok(content) => match serde_json::from_str::<BioRouterApp>(&content) {
    ...
    pub fn store_app(&self, app: &BioRouterApp) -> Result<(), std::io::Error> {
    ...
    pub fn get_app(&self, extension_name: &str, resource_uri: &str) -> Option<BioRouterApp> {
        ...
        fs::read_to_string(&app_path)
            .ok()
            .and_then(|content| serde_json::from_str::<BioRouterApp>(&content).ok())
    }
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Ok(content) = fs::read_to_string(&path) {
                    if let Ok(app) = serde_json::from_str::<BioRouterApp>(&content) {
...
) -> Result<Vec<BioRouterApp>, ErrorData> {
    let mut apps = Vec::new();
