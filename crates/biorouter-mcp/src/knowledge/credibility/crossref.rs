...
mod tests {
    ...
    async fn classifies_peer_reviewed_journal_article() {
        ...
        let c = classify_with(&client, &server.uri(), "10.1038/s41588-024-01893-6")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn classifies_preprint_via_posted_content() {
        ...
        Mock::given(method("GET")).and(path_regex(r"^/works/.*"))
            ...
            .mount(&server).await;
        let client = reqwest::Client::new();
        let c = classify_with(&client, &server.uri(), "10.1101/2024.01.01")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn classifies_book() {
        ...
        Mock::given(method("GET"))
            ...
            .await;
        let client = reqwest::Client::new();
        let c = classify_with(&client, &server.uri(), "10.7551/mitpress/0")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn detects_retraction() {
        ...
        Mock::given(method("GET"))
            ...
            .await;
        let client = reqwest::Client::new();
        let c = classify_with(&client, &server.uri(), "10.1002/x")
            .await
            .unwrap()
            .unwrap();
        assert!(c.retracted);
