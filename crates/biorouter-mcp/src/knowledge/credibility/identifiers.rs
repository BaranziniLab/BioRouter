...
static DOI_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b10\.\d{4,9}/[-._;()/:A-Z0-9]+\b").unwrap());
...
static ISBN_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bISBN(?:-1[03])?:?\s*(97[89][- ]?(?:\d[- ]?){9}\d|(?:\d[- ]?){9}[\dX])\b")
        .unwrap()
});
...
pub fn extract(text: &str) -> Identifiers {
    ...
    Identifiers {
        ...
        arxiv: ARXIV_RE.captures(text).and_then(|c| {
            c.get(1)
                .or_else(|| c.get(2))
                .map(|m| m.as_str().to_string())
        }),
        pmid: PMID_RE.captures(text).and_then(|c| {
            c.get(1)
                .or_else(|| c.get(2))
                .map(|m| m.as_str().to_string())
        }),
