...
async fn diverge_session(
    ...
    // diverge_session does the placeholder-aware, sibling-numbered naming and
    // records the lineage pointer. The full conversation is carried over.
    let new_session = manager
        .diverge_session(&session_id, request.name)
        .await
        .map_err(|e| {
...
mod diverge_tests {
    ...
    async fn diverge_copies_full_history_and_keeps_original() {
        ...
        manager
            .add_message(&original.id, &user_msg("world"))
            .await
            .unwrap();
