...
use biorouter::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("BIOROUTER_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn biorouter::providers::base::Provider>,
            &session.id,
        )
        .await?;
