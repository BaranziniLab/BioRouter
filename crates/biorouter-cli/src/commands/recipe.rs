use anyhow::Result;
use console::style;
use biorouter::recipe::validate_recipe::validate_recipe_template_from_file;
    ...
    match generate_deeplink(recipe_name, params_map) {
        Ok((deeplink_url, recipe)) => match opener(&deeplink_url) {
            Ok(_) => {
                writeln!(
                    out,
                    "{} Opened recipe '{}' in BioRouter Desktop",
                    style("✓").green().bold(),
                    recipe.title
                )?;
                Ok(())
            ...
            Err(err) => {
                writeln!(
                    out,
                    "{} Failed to open recipe in BioRouter Desktop: {}",
                    style("✗").red().bold(),
                    err
                )?;
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("biorouter://recipe?config="));
        let encoded_part = url.strip_prefix("biorouter://recipe?config=").unwrap();
    ...
    fn test_handle_open_opener_fails() {
        ...
        assert!(result.is_err());
        assert!(output.contains("Failed to open recipe in BioRouter Desktop"));
