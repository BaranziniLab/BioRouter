...
BACKUP_DIR="${HOME}/.local/share/biorouter/biorouter-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/biorouter/migrations"
RUST_SESSION_MANAGER="crates/biorouter/src/session/session_manager.rs"
...
get_biorouter_db_path() {
    if [[ -n "${BIOROUTER_PATH_ROOT:-}" ]]; then
        echo "${BIOROUTER_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/biorouter/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/biorouter/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== BioRouter Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run BioRouter yet."
        echo "Once you run BioRouter, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the biorouter repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}BioRouter Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/biorouter/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
