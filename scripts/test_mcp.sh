...
JUDGE_PROVIDER=${BIOROUTER_JUDGE_PROVIDER:-openrouter}
JUDGE_MODEL=${BIOROUTER_JUDGE_MODEL:-google/gemini-2.5-flash}
...
for provider_config in "${PROVIDERS[@]}"; do
  ...
  for i in $(seq 1 $((${#PARTS[@]} - 1))); do
    ...
    echo ""
    TMPFILE=$(mktemp)
    (cd "$TESTDIR" && "$SCRIPT_DIR/target/release/biorouter" run --text "Use the sampleLLM tool to ask for a quote from The Great Gatsby" --with-extension "npx -y @modelcontextprotocol/server-everything@2026.1.14" 2>&1) | tee "$TMPFILE"
    echo ""
...
EOF
      ...
      JUDGE_OUT=$(BIOROUTER_PROVIDER="$JUDGE_PROVIDER" BIOROUTER_MODEL="$JUDGE_MODEL" \
        "$SCRIPT_DIR/target/release/biorouter" run --text "$JUDGE_PROMPT" 2>&1)
