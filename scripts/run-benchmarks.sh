#!/usr/bin/env bash
# run-benchmarks.sh - Script to run biorouter benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting BIOROUTER_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets BIOROUTER_TOOLSHIM_MODEL)"
...
BIOROUTER_CMD="biorouter"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/biorouter" ]; then
    BIOROUTER_CMD="./target/debug/biorouter"
    echo "Using debug binary: $BIOROUTER_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/biorouter. Falling back to system-installed biorouter."
  fi
else
  if [ -f "./target/release/biorouter" ]; then
    BIOROUTER_CMD="./target/release/biorouter"
    echo "Using release binary: $BIOROUTER_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/biorouter. Falling back to system-installed biorouter."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export BIOROUTER_PROVIDER="$provider"
  export BIOROUTER_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export BIOROUTER_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export BIOROUTER_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $BIOROUTER_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
