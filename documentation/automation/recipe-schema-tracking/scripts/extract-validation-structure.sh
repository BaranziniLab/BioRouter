...
VERSION=${1:-"main"}
BIOROUTER_REPO=${BIOROUTER_REPO:-"$HOME/Development/biorouter"}

if [ ! -d "$BIOROUTER_REPO" ]; then
    echo "Error: BIOROUTER_REPO directory not found: $BIOROUTER_REPO" >&2
    exit 1
fi
...
if [ "$VERSION" = "main" ]; then
    MOD_RS_CONTENT=$(cat crates/biorouter/src/recipe/mod.rs)
else
    MOD_RS_CONTENT=$(git show "$VERSION:crates/biorouter/src/recipe/mod.rs" 2>/dev/null || {
...
if [ "$VERSION" = "main" ]; then
    if [ -f crates/biorouter/src/recipe/validate_recipe.rs ]; then
        VALIDATE_RS_CONTENT=$(cat crates/biorouter/src/recipe/validate_recipe.rs)
    else
        VALIDATE_RS_CONTENT=""
    fi
else
    VALIDATE_RS_CONTENT=$(git show "$VERSION:crates/biorouter/src/recipe/validate_recipe.rs" 2>/dev/null || echo "")
fi
