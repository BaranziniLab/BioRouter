...
Automated pipeline for detecting and documenting Recipe schema and validation rule changes between biorouter releases.

## Overview

This automation keeps the [Recipe Reference Guide](https://baranzinilab.github.io/BioRouter/docs/guides/recipes/recipe-reference) synchronized with code changes by:
...
cd output && biorouter run --recipe ../recipes/synthesize-validation-changes.yaml

# 5. Update recipe-reference.md
export RECIPE_REF_PATH=/path/to/recipe-reference.md
biorouter run --recipe ../recipes/update-recipe-reference.yaml
```
...
|----------|----------|---------|-------------|
| `RECIPE_REF_PATH` | No | - | Full path to `recipe-reference.md` file (overrides `BIOROUTER_REPO` construction) |
| `BIOROUTER_REPO` | No | Auto-detect | Path to biorouter repository root |

**Example (for local testing):**
```bash
export RECIPE_REF_PATH=/path/to/local/BioRouter/documentation/docs/guides/recipes/recipe-reference.md
# OR
export BIOROUTER_REPO=/path/to/local/goose
```
...
{
  "validation_files": [
    "crates/BioRouter/src/recipe/validate_recipe.rs",
    "crates/BioRouter/src/agents/types.rs"
  ]
}
```
...
Extracts OpenAPI schema from the biorouter codebase.
...
```bash
cd output
biorouter run --recipe ../recipes/synthesize-validation-changes.yaml
```
...
- `recipe-reference.md` - Target documentation file (path from `RECIPE_REF_PATH` or `BIOROUTER_REPO` env var)
