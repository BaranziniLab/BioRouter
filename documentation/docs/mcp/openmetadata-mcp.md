...
description: Add OpenMetadata MCP Server as a biorouter Extension
---
...
Just describe what you want to accomplish in natural language, and biorouter will help you execute the appropriate OpenMetadata operations safely and efficiently.

## Example Usage

In this example, biorouter looks at the metadata for a Redshift table named `customers` stored in OpenMetadata, particularly its lineage. If the lineage does not show that a Databricks table named `customerinfo` is used to build `customers`, biorouter will add it.
