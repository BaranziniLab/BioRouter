...
description: Add the Square API as a biorouter Extension
---
...
This tutorial will get you started with the [Square MCP server](https://developer.squareup.com/docs/mcp) to enable interactive and automated work for your Square seller account. The Square MCP server is an open source project that allows you to interact with the Square API using biorouter.
...
<Tabs groupId="remote-or-local">
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
     ...
     3. biorouter should open a browser tab to an OAuth permissions page. Double-check which permissions you want to allow, and click `Grant Access`.
     4. It will ask you to login or reauthenticate to Square, and may ask you to confirm the permissions you want to allow.
     5. In biorouter, navigate to the chat
    ...
    <TabItem value="cli" label="biorouter CLI">
      ...
      When you start the next session, biorouter will open a browser where you can grant permissions and sign in to your Square account.
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
    [Launch the installer](biorouter://extension?cmd=npx&arg=square-mcp-server&arg=start&id=mcp_square_api&name=Square%20MCP%20Server&description=Square%20API%20MCP%20Server&env=ACCESS_TOKEN%3DYour%20Access%20Token&env=SANDBOX%3Dtrue)
