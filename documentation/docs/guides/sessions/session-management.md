...
A session is a single, continuous interaction between you and biorouter, providing a space to ask questions and prompt action. This guide covers how to manage the session lifecycle.

## Start Session 

:::info First-time setup
In your first session, biorouter prompts you to [set up an LLM (Large Language Model) provider](/docs/getting-started/installation#set-llm-provider).
:::

<Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
        When you open biorouter, you'll see the session interface ready for use. Just type&mdash;[or speak](/docs/guides/sessions/in-session-actions#voice-dictation "Learn how to enable voice dictation")&mdash;your questions, requests, or instructions directly into the input field, and biorouter will immediately get to work. 
        
        When you're ready to work on a new task, you can start a new session in the same directory or a different one. This directory is where biorouter reads and writes files by default.

        <Tabs>
          <TabItem value="same-directory" label="Same Directory" default>

            To start a session in the same biorouter window:
            1. Click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
            2. Click `Home` in the sidebar
            3. Send your first prompt from the chat box

            To start a session in a new biorouter window:
            1. Click the <AppWindow className="inline" size={16} /> button in the top-left
            2. In the new biorouter window, send your first prompt from the chat box
          ...
          <TabItem value="diff-directory" label="Different Directory">
            ...
            3. Click `Open` to open a new biorouter window for the selected directory
        ...
        :::tip
        On macOS, you can use the biorouter dock icon to quickly start sessions:
            - **Drag and drop** a folder onto the biorouter icon to open a new session in that directory
            - **Right-click** the biorouter icon and select `New Window` to open a new session in your most recent directory
        :::

        You can also use keyboard shortcuts to start a new session or manage biorouter windows.
        ...
        | Keep biorouter Window Always on Top | `Cmd+Shift+T` | `Ctrl+Shift+T` |
        ...
        The session opens to your most recently opened directory in a new biorouter window.

    </TabItem>
    <TabItem value="cli" label="biorouter CLI">
        From your terminal, navigate to the directory from which you'd like to start, and run the [session](/docs/guides/biorouter-cli-commands#session-options) command:
        ```sh
        biorouter session 
        ```

        If you want to interact with biorouter in a web-based chat interface, start a session with the [`web`](/docs/guides/biorouter-cli-commands#web) command:
        ```sh
        biorouter web --open
        ```
    </TabItem>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
        ...
        :::tip
        Session descriptions help you manage multiple biorouter windows. When you're in the biorouter chat interface, session descriptions appear in the `Window` menu and in the Dock (macOS) or taskbar (Windows) menu, making it easy to identify and switch between different biorouter sessions.
        ::: 

    </TabItem>
    <TabItem value="cli" label="biorouter CLI">
        By default, biorouter names your session using the current timestamp in the format `YYYYMMDD_<COUNT>`. If you'd like to provide a specific name, this is where you'd do so. For example to name your session `react-migration`, you would run:

        ```
        biorouter session -n react-migration
        ```
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="biorouter CLI">
        ...
        Your session will be stored in the local SQLite database at `~/.local/share/BioRouter/sessions/sessions.db`.
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
  <TabItem value="ui" label="biorouter Desktop" default>
    ...
    You can use keyboard shortcuts and search bar buttons to search sessions in biorouter Desktop.
    ...
    To search conversation content across all your sessions, ask biorouter directly in any chat session. For example:

    - "Find my earlier conversation about React hooks from last week"
    - "Show me sessions where I worked on database migrations"

    biorouter will search your session history and show relevant conversations with context from matching sessions. 
  ...
  <TabItem value="cli" label="biorouter CLI">
    ...
    To search conversation content across all your sessions, start a biorouter session and ask directly:
    ...
    The [`session list`](/docs/guides/biorouter-cli-commands#session-list-options) subcommand with supported options can be useful for some search operations.
    ...
    Starting with version 1.10.0, biorouter uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files. Legacy `.jsonl` files remain on disk but are no longer managed by biorouter.
    :::
...
<Tabs groupId="interface">
    ...
    1. Click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
    2. Click `History` in the sidebar
    3. Click the session you'd like to resume. biorouter provides [search features](#search-sessions) to help you find the session.
    ...
    <TabItem value="cli" label="biorouter CLI">
        ...
        ```
        biorouter session -r --name <name>
        ```
        For example, to resume the session named `react-migration`, you would run:

        ```
        biorouter session -r --name react-migration
        ```
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="biorouter CLI">
        You can use the [`project`](/docs/guides/biorouter-cli-commands#project) and [`projects`](/docs/guides/biorouter-cli-commands#projects) commands to start or resume sessions from a project, which is a tracked working directory with session metadata. For a complete guide to using Projects, see [Managing Projects Guide](/docs/guides/managing-projects).
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
        ...
        Deleting a session from biorouter Desktop will also delete it from the CLI. This action cannot be undone.
        :::
    ...
    <TabItem value="cli" label="biorouter CLI">
        You can remove sessions using CLI commands. For detailed instructions on session removal, see the [CLI Commands documentation](/docs/guides/biorouter-cli-commands#session-remove-options).
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="biorouter Desktop" default>
        Import complete sessions from JSON files to restore, share, or migrate sessions between biorouter instances. Importing creates a new session with a new ID rather than overwriting existing sessions.

        1. Click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
        2. Click `History` in the sidebar
        3. Click the <Upload className="inline" size={16} /> `Import Session` button in the top-right corner
        4. Select a `.json` session file that was previously exported from biorouter
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="biorouter CLI">
        ...
        From your terminal, run the [`session export`](/docs/guides/biorouter-cli-commands#session-export-options) subcommand:
