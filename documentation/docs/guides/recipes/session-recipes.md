...
description: "Share a biorouter session setup (including tools, goals, and instructions) as a reusable recipe that others can launch with a single click"
---
...
You can turn your current biorouter session into a reusable recipe that includes the tools, goals, and setup you're using right now and package it into a new Agent that others (or future you) can launch with a single click.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="biorouter CLI">
   ...
   settings:                      # Additional settings
     biorouter_provider: $provider    # Provider to use for this recipe
     biorouter_model: $model          # Specific model to use for this recipe
  ...
  <TabItem value="generator" label="Recipe Generator">
    Use the online [Recipe Generator](https://baranzinilab.github.io/BioRouter/recipe-generator) tool to create a recipe. First choose your preferred format:
...
<Tabs groupId="interface">
  ...
  Using a recipe with the biorouter CLI might involve the following tasks:
  ...
  Recipes can be stored locally on your device or in a GitHub repository. Configure your recipe repository using either the `biorouter configure` command or [config file](/docs/guides/config-files#global-settings).
  ...
  :::
   ...
   <Tabs>
     <TabItem value="configure" label="Using biorouter configure" default>
       ...
       │  ○ Toggle Experiment 
       // highlight-start
       │  ● biorouter recipe github repo (biorouter will pull recipes from this repo if not found locally.)
       // highlight-end
       └  
       ┌  biorouter-configure 
       │
     ...
     <TabItem value="config" label="Using config file">
       ...
       BIOROUTER_RECIPE_GITHUB_REPO: "owner/repo"
       ```
   ...
   <Tabs groupId="interface">
     <TabItem value="local" label="Local Recipe" default>
       ...
       **Basic Usage** - Run once and exit (see [run options](/docs/guides/biorouter-cli-commands#run-options) and [recipe commands](/docs/guides/biorouter-cli-commands#recipe) for more):
       ```sh
       # Using recipe file in current directory or [`BIOROUTER_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) directories
       biorouter run --recipe recipe.yaml

       # Using full path
       biorouter run --recipe ./recipes/my-recipe.yaml
       ```

       **Preview Recipe** - Use the [`explain`](/docs/guides/biorouter-cli-commands#run-options) command to view details before running:
 
       **Interactive Mode** - Start an interactive session:
       ```sh
       biorouter run --recipe recipe.yaml --interactive
       ```
       ...
       **With Parameters** - Supply parameter values when running recipes. See the [`run` command documentation](/docs/guides/biorouter-cli-commands#run-options) for detailed examples and options.

       Basic example:
       ```sh
       biorouter run --recipe recipe.yaml --params language=Python
       ```
     ...
     <TabItem value="github" label="GitHub Recipe">
       ...
       **Basic Usage** - Run recipes from your configured repo using the recipe name that matches its directory (see [run options](/docs/guides/biorouter-cli-commands#run-options) and [recipe commands](/docs/guides/biorouter-cli-commands#recipe) for more):

       ```sh
       biorouter run --recipe recipe-name
       ```
       ...
       ```sh
       biorouter run --recipe code-review
       ```
      ...
      **Preview Recipe** - Use the [`explain`](/docs/guides/biorouter-cli-commands#run-options) command to view details before running:
       ...
       **Interactive Mode** - With parameter prompts:
       ```sh
       biorouter run --recipe code-review --interactive
       ```
       The interactive mode will prompt for required values:
       ```sh
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="biorouter CLI">
   ...
   ```sh
   biorouter recipe validate recipe.yaml
   ```

   :::info
   If you want to validate a recipe you just created, you need to [exit the session](/docs/guides/sessions/session-management#exit-session) before running the [`validate` subcommand](/docs/guides/biorouter-cli-commands#recipe).
   :::
...
## Share Recipe
Share your recipe with biorouter users using a recipe link or recipe file.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="biorouter CLI">
    ...
    ```sh
    biorouter recipe deeplink <FILE>
    ```

    You can also provide parameter values to pre-fill the `Recipe Parameters` dialog:
    ```sh
    biorouter recipe deeplink <FILE> --param key1=value1 --param key2=value2
    ```
  </TabItem>
</Tabs>

When someone clicks the link, it will open biorouter Desktop with your recipe configuration. They can also use your recipe link to [import a recipe](/docs/guides/recipes/storing-recipes#importing-recipes) for future use.
...
- CLI users can run a YAML or JSON recipe using `biorouter run --recipe <FILE>` or open it directly in biorouter Desktop with `biorouter recipe open <FILE>`. See the [CLI Commands guide](/docs/guides/biorouter-cli-commands#recipe) for details.

## Schedule Recipe
<Tabs groupId="interface">
  <TabItem value="ui" label="biorouter Desktop" default>
Automate biorouter recipes by running them on a schedule. When creating a schedule, you'll configure:
- **Name**: A descriptive name for the schedule
- **Source**: The recipe to run
- **Execution mode**: Whether the recipe runs in the background (no window, results saved) or foreground (opens window if biorouter Desktop is running, otherwise runs in background)
- **Frequency and time**: When to run the recipe (e.g. every 20 minutes, weekly at 10 AM on Friday). Your selection is converted into a [cron expression](https://en.wikipedia.org/wiki/Cron#Cron_expression) used by biorouter.
...
At the bottom of the `Schedule Details` page you can view the list of sessions created by the scheduled recipe and open or restore each session.
  ...
  <TabItem value="cli" label="biorouter CLI">
  Automate biorouter recipes by scheduling them to run with a [cron expression](https://en.wikipedia.org/wiki/Cron#Cron_expression).

  ```bash
  # Add a new scheduled recipe which runs every day at 9 AM
  biorouter schedule add --schedule-id daily-report --cron "0 0 9 * * *" --recipe-source ./recipes/daily-report.yaml
  ```
...
biorouter run --recipe analysis.yaml --params project_path=./src > output.log
...
:::info
Structured output is supported in recipes run in both the biorouter CLI and biorouter Desktop. However, creating and editing the `json_schema` configuration must be done manually in the recipe file.
:::
...
## Learn More
Check out the [Recipes](/docs/guides/recipes) guide for more docs, tools, and resources to help you master biorouter recipes.
