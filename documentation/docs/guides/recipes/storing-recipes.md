...
This guide covers storing, organizing, and finding biorouter recipes when you need to access them again later. 

:::info Desktop UI vs CLI
- **biorouter Desktop** has a visual Recipe Library for browsing and managing saved recipes
- **biorouter CLI** stores recipes as files that you find using file paths or environment variables
:::
...
|------|----------|-------------|----------|
| **Global** | `~/.config/BioRouter/recipes/` | All projects and sessions | Personal workflows, general-purpose recipes |
| **Local** | `YOUR_WORKING_DIRECTORY/.biorouter/recipes/` | Only when working in that project | Project-specific workflows, team recipes |
...
<Tabs groupId="interface">
  <TabItem value="desktop" label="biorouter Desktop" default>
...
:::
  ...
  <TabItem value="cli" label="biorouter CLI">
    ...
    * Local project recipes: `/recipe .biorouter/recipes/my-recipe.yaml`
...
<Tabs groupId="interface">
  ...
  biorouter Desktop accepts `.yaml`, `.yml`, and `.json` files, but **the CLI only supports `.yaml` and `.json`**. For full compatibility across both interfaces, avoid `.yml` extensions.
  ...
  <TabItem value="cli" label="biorouter CLI">
    Recipe import is only available in biorouter Desktop.
  </TabItem>
</Tabs>
...
Use the `biorouter recipe list` command to find all available recipes from multiple sources:
...
biorouter searches for recipes in the following locations (in order):

1. **Current directory**: `.` (looks for `*.yaml` and `*.json` files)
2. **Custom paths**: Directories specified in [`BIOROUTER_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) environment variable
3. **Global recipe library**: `~/.config/BioRouter/recipes/` (or equivalent on your OS)
4. **Local project recipes**: `./.biorouter/recipes/`
5. **GitHub repository**: If [`BIOROUTER_RECIPE_GITHUB_REPO`](/docs/guides/environment-variables#recipe-configuration) environment variable is configured
...
$ biorouter recipe list
Available recipes:
biorouter-self-test - A comprehensive meta-testing recipe - local: ./biorouter-self-test.yaml
hello-world - A sample recipe demonstrating basic usage - local: ~/.config/BioRouter/recipes/hello-world.yaml
job-finder - Find software engineering positions - local: ~/.config/BioRouter/recipes/job-finder.yaml
```
...
Available recipes:
  ...
  hello-world - A sample recipe demonstrating basic usage - local: ~/.config/BioRouter/recipes/hello-world.yaml
    ...
    Path: /Users/username/.config/BioRouter/recipes/hello-world.yaml
```
...
ls ~/.config/BioRouter/recipes/

# List recipes in current project
ls .biorouter/recipes/
...
:::tip
The `biorouter recipe list` command is the recommended way to find recipes as it automatically searches all configured sources and provides consistent formatting.
:::
...
Once you've located your recipe file, [run the recipe](/docs/guides/recipes/session-recipes#run-a-recipe) or [open it in biorouter Desktop](/docs/guides/biorouter-cli-commands#recipe).

:::tip Format Compatibility
The CLI can run recipes saved from biorouter Desktop without any conversion. Both CLI-created and Desktop-saved recipes work with all recipe commands.
:::
