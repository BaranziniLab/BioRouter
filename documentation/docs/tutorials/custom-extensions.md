...
description: Create your own custom MCP Server to use as a biorouter extension
---
...
:::info
biorouter supports Tools, Resources, and Prompts from the [Model Context Protocol](https://modelcontextprotocol.io/). See [`mcp_client.rs`](https://github.com/BaranziniLab/BioRouter/blob/main/crates/BioRouter/src/agents/mcp_client.rs) for the supported protocol version and client capabilities.
:::
...
## Step 5: Integrate with biorouter

To add your MCP server as an extension in biorouter:
...
2. Open biorouter Desktop and click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
...
To see any changes you make to your MCP server code after integrating with biorouter, re-run `uv pip install .` and then restart biorouter Desktop.
:::
...
Once integrated, you can start using your extension in biorouter. Open the biorouter chat interface and call your tool as needed.

You can verify that biorouter has picked up the tools from your custom extension by asking it "what tools do you have?"
...
🎉 **Congratulations!** You’ve successfully built and integrated a custom MCP server with biorouter.

---

## Advanced Features for MCP Extensions

biorouter supports advanced MCP features that can enhance your extensions.

### MCP Sampling: AI-Powered Tools

**[MCP Sampling](/docs/guides/mcp-sampling)** allows your MCP servers to request AI completions from biorouter's LLM, transforming simple tools into intelligent agents.
...
- [biorouter's implementation](https://github.com/BaranziniLab/BioRouter/blob/main/crates/BioRouter/src/agents/mcp_client.rs) currently supports text and image content types
- biorouter automatically advertises sampling capability to all MCP servers
...
- Components render securely in isolated sandboxes within biorouter Desktop
...
:::note
biorouter also supports [MCP-UI](/docs/guides/interactive-chat/mcp-ui), but MCP Apps is the recommended path for new extensions.
:::
