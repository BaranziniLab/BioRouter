...
description: Create interactive UI applications that render inside biorouter Desktop
---
...
MCP Apps let MCP servers return interactive UIs that render directly inside the biorouter chat interface, rather than responding with text alone. This allows users to express intent through interaction, which is useful for workflows that require input, iteration, or visual feedback.

:::warning Experimental
MCP Apps support in biorouter is experimental and based on a draft specification. The implementation is minimal and may change, and does not yet support advanced capabilities or persistent app windows.
:::
...
- biorouter Desktop 1.19.1+ installed
:::
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "show_demo_app") {
    return {
      ...
      // This metadata tells biorouter to render the MCP App
      _meta: {
        ui: {
...
<body class="light">
  <div class="container">
    ...
    <div class="message-section">
      <h3>💬 Send a message to biorouter</h3>
    ...
    <div class="info-section">
      ...
      It communicates with biorouter via JSON-RPC messages through the sandbox bridge.
      <br><br>
      • Counter uses local state<br>
      • "Send" calls <code>ui/message</code> to append text to chat<br>
      • Theme syncs with biorouter's theme setting
    </div>
  </div>
...
## Step 4: Add to biorouter Desktop
...
1. Restart biorouter to load the new extension
2. Prompt biorouter: "Show me the demo app"
3. biorouter will call the `show_demo_app` tool
4. Your interactive app will render in the chat!

Try:
- Clicking the counter buttons
- Typing a message and clicking "Send"
- Switching biorouter between light/dark mode
...
└──────────────────┬───────────────────┘
                   │ postMessage
┌──────────────────▼───────────────────┐
│          biorouter Desktop               │  Renders UI, routes messages
└──────────────────┬───────────────────┘
                   │ MCP Protocol
┌──────────────────▼───────────────────┐
...
Your server returns a `ui://` resource URI, biorouter fetches the HTML and renders it in an iframe. The app communicates back via `postMessage`—requesting theme info, sending messages to chat, or resizing itself.
