---
title: Advanced Cognee Usage with biorouter
description: Advanced patterns for using Cognee knowledge graph with biorouter for enhanced memory and automation
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Advanced Cognee Usage with biorouter

This tutorial covers advanced usage patterns for the Cognee extension with biorouter, including automated memory management, knowledge graph optimization, and various integration strategies.

## Overview

While the basic [Cognee MCP setup](../mcp/cognee-mcp.md) gets you started, this tutorial explores how to make biorouter autonomously use the knowledge graph and optimize your workflow.
...
Create `~/.config/BioRouter/cognee-instructions.md`:
...
Start biorouter with instructions:
```bash
biorouter run -i ~/.config/BioRouter/cognee-instructions.md -s
```
...
For faster startup with higher token usage, add to your `.biorouterhints` file:
...
3. Lower token usage than biorouterhints
...
```bash
# Start biorouter with Cognee
biorouter session

# In biorouter, analyze your codebase
> biorouter, please codify this repository and then help me understand the architecture
```
...
> biorouter, please cognify the contents of these research papers: paper1.pdf, paper2.pdf, paper3.pdf
...
```bash
# Check status
> biorouter, what's the status of the cognify pipeline?

# Selective pruning (if needed)
> biorouter, can you help me identify outdated information in the knowledge graph?
```
...
4. **High token usage**: Use instruction files instead of biorouterhints
...
# In biorouter session:
> biorouter, run cognify_status and codify_status
```
...
1. **Use nodesets** for organizing different types of information:
   ```bash
   # Developer rules
   > biorouter, add these coding standards to the 'developer_rules' nodeset
   
   # Project-specific info  
   > biorouter, cognify this project documentation with nodeset 'project_alpha'
   ```
...
```bash
# Setup
> biorouter, codify this repository and remember that I prefer: functional programming patterns, comprehensive tests, and clear documentation
...
```bash
# Before meeting
> biorouter, cognify the agenda and participant backgrounds from these documents
...
```bash
# Literature review
> biorouter, cognify these 10 research papers and create a knowledge graph of the relationships between their methodologies

# Synthesis
> What are the emerging patterns in the research and what gaps exist?
```

This advanced usage guide should help you maximize the potential of Cognee with biorouter for sophisticated knowledge management and automation workflows.
