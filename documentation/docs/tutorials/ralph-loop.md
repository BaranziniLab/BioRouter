---
title: Ralph Loop
description: Run biorouter in a loop with fresh context per iteration and cross-model review
---

Ralph Loop, based on [Geoffrey Huntley's Ralph Wiggum technique](https://ghuntley.com/ralph/), is an iterative development pattern that keeps biorouter working on a task until it's genuinely complete.
...
- [Install the biorouter CLI](/docs/getting-started/installation) because the Ralph Loop runs via the terminal.
...
```bash
mkdir -p ~/.config/BioRouter/recipes

curl -sL https://raw.githubusercontent.com/block/BioRouter/main/documentation/src/pages/recipes/data/recipes/ralph-loop.sh -o ~/.config/BioRouter/recipes/ralph-loop.sh
curl -sL https://raw.githubusercontent.com/block/BioRouter/main/documentation/src/pages/recipes/data/recipes/ralph-work.yaml -o ~/.config/BioRouter/recipes/ralph-work.yaml
curl -sL https://raw.githubusercontent.com/block/BioRouter/main/documentation/src/pages/recipes/data/recipes/ralph-review.yaml -o ~/.config/BioRouter/recipes/ralph-review.yaml

chmod +x ~/.config/BioRouter/recipes/ralph-loop.sh
```
...
```bash
~/.config/BioRouter/recipes/ralph-loop.sh "Create a simple browser using Electron and React"
```
...
```bash
~/.config/BioRouter/recipes/ralph-loop.sh ./prd.md
```
:::
...
|--------|-------------|
| Worker model | The model that does the actual coding work. Defaults to `BIOROUTER_MODEL` if set. |
| Worker provider | The provider for the worker model (e.g., `openai`, `anthropic`). Defaults to `BIOROUTER_PROVIDER` if set. |
...
The terminal will show biorouter moving through the worker and reviewer phases. Each iteration starts with a fresh session to keep the context clean. Here's what a successful run looks like:
...
▶ WORK PHASE
... (biorouter creates initial implementation) ...

▶ REVIEW PHASE
... (biorouter reviews the work) ...
...
▶ WORK PHASE
... (biorouter addresses feedback) ...

▶ REVIEW PHASE
... (biorouter reviews again) ...
...
#   RALPH_RECIPE_DIR      - Recipe directory (default: ~/.config/BioRouter/recipes)
#

set -e

INPUT="$1"
RECIPE_DIR="${RALPH_RECIPE_DIR:-$HOME/.config/BioRouter/recipes}"
...
prompt_for_settings() {
    local default_model="${BIOROUTER_MODEL:-}"
    local default_provider="${BIOROUTER_PROVIDER:-}"
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    BIOROUTER_PROVIDER="$WORKER_PROVIDER" BIOROUTER_MODEL="$WORKER_MODEL" biorouter run --recipe "$RECIPE_DIR/ralph-work.yaml" || {
    ...
    BIOROUTER_PROVIDER="$REVIEWER_PROVIDER" BIOROUTER_MODEL="$REVIEWER_MODEL" biorouter run --recipe "$RECIPE_DIR/ralph-review.yaml" || {
