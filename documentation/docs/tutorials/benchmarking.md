---
title: Benchmarking with biorouter
sidebar_label: Benchmark with biorouter
---

The biorouter benchmarking system allows you to evaluate biorouter performance on complex tasks with one or more system
configurations.<br></br>
This guide covers how to use the `biorouter bench` command to run benchmarks and analyze results.
...
```bash
biorouter bench selectors
```

2. Create a basic configuration file:

```bash
biorouter bench init-config -n bench-config.json
...
```bash
biorouter bench run -c bench-config.json
```
...
```bash
RUST_LOG=debug biorouter bench bench-config.json
```
...
Tool shimming allows you to use a non-tool-capable models with biorouter, provided Ollama is installed on the
system.
