...
description: Discover how biorouter can study your past sessions and turn your favorite workflows into reusable automation templates.
authors: 
    - ian
---
...
You've been using biorouter for weeks, maybe months. You have dozens of successful [sessions](/docs/guides/sessions/) where you asked for help with blog posts, code reviews, documentation, or data analysis. Each time you think "Didn't I already do this?" but never get around to checking. Sound familiar?

I myself had over a hundred biorouter sessions and as many megabytes of conversation data. I was sitting on a goldmine of potential automation. A coworker suggested something brilliant: "What if biorouter could analyze your sessions and build recipes automatically?" Wait, wait, wait!! Create a personalized cookbook based on my own session history? Yes, please! Let's build a "cookbook generator" recipe!
...
Let's boost our productivity by implementing some automation. We're gonna get biorouter to write a recipe that creates OTHER recipes!

## What is the Cookbook Generator Recipe?

The "cookbook generator" I'm describing here is a way to get biorouter to look at your previous sessions, analyze them for commonalities, and automatically create new recipes from common patterns. It's automation that creates automation -- meta-programming at its finest.

I'm going to share my own cookbook generator at the end.

Here's what it does:

1. **Scans your session history** -- Finds and reads all your biorouter session files
...
[I demonstrated on a livestream](https://www.youtube.com/watch?v=-_1GALH2ER0) my whole process of creating this generator using "[vibe prompting](https://www.youtube.com/watch?v=IjXmT0W4f2Q)" -- having an extended conversation with biorouter to refine the idea and answer potential questions upfront. This approach uses fewer tokens than iterative coding, resulting in higher success rates.

<iframe class="aspect-ratio" src="https://www.youtube.com/embed/-_1GALH2ER0" title="Vibe Code with biorouter: Automate my workflow" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

During my own conversation with biorouter, biorouter asked smart questions like:
...
The generator reads through your session files (typically stored in `~/.local/share/BioRouter/sessions/` but this may vary on your platform, and may change in future biorouter releases) and analyzes:
- Overall outcome success
- User request patterns
- Tool usage sequences
- Common parameter types it could extract

This last one was really important for me. I ask biorouter all the time to help me build an outline for blog posts like this, or video scripts for our [YouTube channel](https://youtube.com/@biorouter-oss), or tutorial pages. They all follow a similar pattern, around a topic or subject, but the output format may vary.
...
:::tip
I wanted biorouter to write all the recipes for me, but to be as up-to-date as possible. I cloned the [BioRouter repository](https://github.com/BaranziniLab/BioRouter) and told biorouter to examine its own source code to learn how to successfully create recipes, and be sure to use proper YAML syntax.
:::

From that, I had biorouter think about the following ideas as it considered how to make my recipes:
...
Building the cookbook generator took a little over an hour of "vibe prompting" and refining the idea, and then biorouter gave me a recipe. I always verify my AI-generated work, and then spent about 15 minutes more after the livestream refining some ideas and adding more guardrails to the recipe.
...
- **Social Media Posts**: I struggle a lot with coming up with catchy social media posts, so I want to give biorouter the kind of content (blog, video, etc), point it at the content (in the case of video, I give it my narration script), and have it generate many different options that I can choose from later.
...
By running the outline generator recipe (three times), the image generator recipe, and social media recipe, biorouter took care of the following work:
...
- the script outline for a longer video going on our [YouTube channel](https://youtube.com/@biorouter-oss)
...
The last thing I'd want to improve would be to have biorouter re-analyze the recipes it made with newer sessions to refine existing recipes, instead of creating new recipes each time it runs.
...
The key insight is that AI agents shouldn't just execute tasks -- they should learn from those executions and help us build better systems. This generator turns your biorouter usage history into a productivity asset, creating a personalized automation toolkit from your actual work patterns.
...
Want to contribute your own recipes or improvements to the cookbook generator? Join our [Discord community](https://discord.gg/biorouter-oss) or check out our [GitHub repository](https://github.com/BaranziniLab/BioRouter) for more automation ideas. Join our [Hacktoberfest](https://github.com/BaranziniLab/BioRouter/issues/4705) event going on to contribute recipes and prompt ideas to get on our leaderboard to win some great prizes!


## My Own Cookbook Generator Recipe

Here's the cookbook generator that biorouter helped me create, plus my own notes. You could try using it as-is, but I think a better approach would be to try vibe prompting with biorouter yourself to go analyze your own session history to see what kind of automation you want to set up for yourself.

```yaml
version: "1.0.0"
title: Recipe Cookbook Generator
description: |
  Analyze your biorouter session history to automatically generate recipes from your common workflows. This tool examines your past interactions with biorouter, identifies repetitive patterns, and creates reusable recipes that can automate similar tasks in the future. Perfect for capturing your personal automation patterns and building a custom recipe library.

prompt: |
  I want to analyze my biorouter session history and create recipes from common workflows I've used. I've done a variety of work, and I'd like your help finding repetitive tasks that we can turn into biorouter recipes to build my own personal 'cookbook' based on my biorouter usage patterns.
  ...
  The process should:
  1. Find and analyze my biorouter session files{% if session_storage_path %} (located at {{ session_storage_path }}){% endif %}
...
parameters:
  ...
  - key: session_storage_path
    description: Path to biorouter session storage (will auto-detect if not provided)
    input_type: string
...
instructions: |
  You are helping the user build a 'cookbook' of biorouter recipes based on their actual usage patterns.
  
  ## Step 1: Setup and Discovery
  - Determine the biorouter session storage path (try default locations like ~/.local/share/BioRouter/sessions, or ask the user)
  ...
  - Each recipe includes:
    - Proper biorouter YAML format with version, title, description
...
<head>
  ...
  <meta property="og:url" content="https://baranzinilab.github.io/BioRouter/blog/2025/10/08/recipe-cookbook-generator" />
  <meta property="og:description" content="Discover how biorouter can study your past sessions and turn your favorite workflows into reusable automation templates." />
  <meta property="og:image" content="https://baranzinilab.github.io/BioRouter/assets/images/recipe-cookbook-generator-ba6ce396585e1fc591e339720ee80a30.png" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="Build Your Own Recipe Cookbook Generator for goose" />
  <meta name="twitter:description" content="Discover how biorouter can study your past sessions and turn your favorite workflows into reusable automation templates." />
  <meta property="twitter:domain" content="baranzinilab.github.io/BioRouter" />
  <meta name="twitter:image" content="https://baranzinilab.github.io/BioRouter/assets/images/recipe-cookbook-generator-ba6ce396585e1fc591e339720ee80a30.png" />
</head>
