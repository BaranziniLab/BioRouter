...
server.addTool({
  name: "fetchYoutube",
  description: "Fetch ALL YouTube videos from the biorouter channel.",
  parameters: z.object({}),
...
Same pattern for blogs and GitHub releases, straightforward tool functions with clear descriptions. The key is making your tool descriptions super simple and direct. biorouter needs to know exactly what each tool does.
...
Once both MCP servers were built, I plugged them into biorouter. For local servers, you just:
...
Then I asked biorouter: "Hey, can you tell me if we have any new content?"
...
Once both MCP servers were built, I still needed something to pull them together. MCP servers do not talk to each other on their own. Without biorouter and an orchestrating recipe, they are just two separate tools waiting to be called.
...
In biorouter Desktop, open the `recipe` section, click the `calendar icon` , and choose when it should run (I set mine to 10 AM daily).

You can read more in the [Shareable Recipes Guide](https://baranzinilab.github.io/BioRouter/docs/guides/recipes/session-recipes#schedule-recipe).
:::
...
description: "Fetches new biorouter content or posts evergreen, generates platform-specific captions, and creates Sprout drafts."
...
instructions: |
  ...
  **IF NO new content exists:**
  - Load the file /Users/ebonyl/.config/BioRouter/evergreen.json
  ...
  **IF the item was NEW content (not evergreen):**
  ...
  - This updates ~/.config/BioRouter/content-fetcher-mcp/last_seen.json
...
activities:
  - "Fetching latest biorouter content from all sources"
...
The best part? You can take this same approach for whatever automation you need. Spin up some MCP servers, create a recipe, let biorouter handle the orchestration. It's honestly so much fun watching it all come together.
...
<head>
  ...
  <meta property="og:url" content="https://baranzinilab.github.io/BioRouter/blog/2025/11/21/building-social-media-agent" />
  ...
  <meta property="og:image" content="https://baranzinilab.github.io/BioRouter/assets/images/header-image-7f5ab50f65332fb53302ca30a3f86e46.png" />
  ...
  <meta name="twitter:image" content="https://baranzinilab.github.io/BioRouter/assets/images/header-image-7f5ab50f65332fb53302ca30a3f86e46.png" />
</head>