...
description: How BioRouter uses multiple LLMs within a single task, optimizing for speed, cost, and reliability in AI agent workflows
authors:
    - mic
    - angie
---
...
Not just switching based on user commands, but building BioRouter with an actual system for routing tasks between different models, each playing to their strengths.
...
Originally, every BioRouter session used a single model from start to finish. That worked fine for short tasks, but longer sessions were harder to tune:
...
* If the worker gets stuck, BioRouter can detect the failure and temporarily bring the lead back in.


You can configure how many turns the lead handles upfront (`BIOROUTER_LEAD_TURNS`), how many consecutive failures trigger fallback (`BIOROUTER_LEAD_FAILURE_THRESHOLD`), and how long the fallback lasts before BioRouter retries the worker.
...
We didn't want BioRouter to swap models just because an API timed out. Instead, we focused on real task failures:
...
BioRouter tracks these signals and knows when to escalate. And once the fallback model stabilizes things, it switches back without missing a beat.
...
It also opens the door for even smarter routing in the future with things like switching based on tasks, ensemble voting, or maybe even letting BioRouter decide which model to call based on tool context.

## Try It Out

Lead/worker mode is already available in BioRouter.  To enable, export these variables with two models that have already been configured in BioRouter:

```bash
export BIOROUTER_LEAD_MODEL="gpt-4o"
export BIOROUTER_MODEL="claude-4-sonnet"
```

From there, BioRouter takes care of the hand off, the fallback, and the recovery. You just... keep vibing.

If you're curious how it all works under the hood, we've got a [full tutorial](/docs/tutorials/lead-worker).

---

If you're experimenting with multi-model setups, [share what's working and what isn't](https://discord.gg/biorouter-oss).


<head>
  <meta property="og:title" content="Treating LLMs Like Tools in a Toolbox: A Multi-Model Approach to Smarter AI Agents" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://baranzinilab.github.io/BioRouter/blog/2025/06/16/multi-model-in-biorouter" />
  <meta property="og:description" content="How BioRouter uses multiple LLMs within a single task, optimizing for speed, cost, and reliability in AI agent workflows" />
  <meta property="og:image" content="https://baranzinilab.github.io/BioRouter/assets/images/multi-model-ai-agent-d408feaeba3e13cafdbfe9377980bc3d.png" />
  ...
  <meta name="twitter:description" content="How BioRouter uses multiple LLMs within a single task, optimizing for speed, cost, and reliability in AI agent workflows" />
  <meta name="twitter:image" content="https://baranzinilab.github.io/BioRouter/assets/images/multi-model-ai-agent-d408feaeba3e13cafdbfe9377980bc3d.png" />
</head>