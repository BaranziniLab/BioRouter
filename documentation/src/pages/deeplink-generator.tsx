...
export default function DeeplinkGenerator() {
  ...
  useEffect(() => {
    const sampleJson = {
      ...
      description: "An example BioRouter extension",
  ...
  useEffect(() => {
    ...
    if (urlParams.toString()) {
      try {
        ...
        if (urlParams.get('cmd') === 'biorouterd' && urlParams.getAll('arg').includes('mcp')) {
  ...
  const generateDeeplink = (server: ServerConfig): string => {
    if (server.is_builtin) {
      ...
      return `biorouter://extension?${queryParams}`;
    }
      ...
      <div className="container mx-auto px-4 py-8 md:p-24">
        <div className="pb-8 md:pb-16">
          ...
          <p className="text-textProminent">
            Generate installation deeplinks for BioRouter extensions that can be shared with others.
          </p>
        </div>
        ...
        <div className="mt-8 bg-bgApp border border-borderSubtle rounded-lg p-6 shadow-sm">
          ...
          <ol className="list-decimal pl-6 space-y-2 text-textStandard">
            ...
            <li>Copy and share the generated deeplink - when users click it, it will open BioRouter Desktop and prompt them to install your extension.</li>
          </ol>
        </div>
      </div>
    </Layout>
