...
const FALLBACK_URL = "https://github.com/BaranziniLab/BioRouter/releases/latest";
...
const LinuxDesktopInstallButtons = () => {
  ...
  useEffect(() => {
    const fetchLatestRelease = async () => {
      try {
        ...
        const cached = localStorage.getItem('biorouter-release-cache');
        const cacheTime = localStorage.getItem('biorouter-release-cache-time');
        const now = Date.now();
        ...
        const response = await fetch('https://api.github.com/repos/block/BioRouter/releases/latest');
        ...
        localStorage.setItem('biorouter-release-cache', JSON.stringify(newUrls));
        localStorage.setItem('biorouter-release-cache-time', now.toString());
      } catch (error) {
  ...
  return (
    <div>
      <p>Click one of the buttons below to download biorouter Desktop for Linux:</p>
