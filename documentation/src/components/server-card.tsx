...
import { getBioRouterInstallLink } from "@site/src/utils/install-links";
...
const getExtensionCommand = (server: MCPServer): string => {
  switch (server.type) {
    case "remote":
      return `biorouter session --with-remote-extension "${server.url}"`;
    case "streamable-http":
      return `biorouter session --with-streamable-http-extension "${server.url}"`;
    case "local":
    default:
      return `biorouter session --with-extension "${server.command}"`;
  }
};
              ...
              <div className="card-action">
                {server.is_builtin ? (
                  <div
                    ...
                    title="This extension is built into biorouter and can be enabled in the settings page"
                  >
                    Built-in
                  </div>
                ) : server.show_install_link !== false ? (
                  <a
                    href={getBioRouterInstallLink(server)}
                    target="_blank"
