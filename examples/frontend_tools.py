...
BIOROUTER_PORT = "3001"
BIOROUTER_URL = f"http://{BIOROUTER_HOST}:{BIOROUTER_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{BIOROUTER_URL}/agent/update_provider",
            json={"provider": "databricks", "model": "biorouter"},
        ...
        response = await client.post(
            f"{BIOROUTER_URL}/extensions/add",
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to BioRouter.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with BioRouter."""
