...
biorouter can autonomously decide to use subagents when it determines they would be beneficial for your task - you don't always need to explicitly request them. This happens automatically in autonomous [permission mode](/docs/guides/biorouter-permissions) (the default). Subagents are disabled in manual approval, smart approval, and chat-only modes.
:::
...
**Place your recipe file where biorouter can find it**
- Set [`BIOROUTER_RECIPE_PATH`](/docs/guides/recipes/recipe-reference#recipe-location) environment variable to your recipe directory
...
**[goose Configuration File](/docs/guides/config-files)** (`.~/.config/BioRouter/config.yaml `):
```yaml
subagent:
  args:
  - mcp-server
...
**Environment variable:** Set `BIOROUTER_SUBAGENT_MAX_TURNS` to change the default max turns for all subagents.
  ...
  items={[
    {
      ...
      description: 'Deep dive into biorouter subagents. Walk through building an app using 6 specialized AI agents for advanced workflow automation and development.',
    ...
    },
    {
      ...
      description: 'Discover the differences between subagents and subrecipes for efficient task execution in biorouter. Learn which approach is best for your workflow.',
    ...
    },
   {
      ...
      description: 'Detailed guide to subagents and subrecipes in biorouter. Compare reusability, setup complexity, and get practical advice for choosing the right approach.',
      thumbnailUrl: subagentsVsSubrecipes,
      linkUrl: '/BioRouter/blog/2025/09/26/subagents-vs-subrecipes',
      date: '2025-09-26',
      duration: '6 min read'
    },
    {
      ...
      linkUrl: '/BioRouter/blog/2025/08/14/agent-coordination-patterns',
      date: '2025-08-14',
      duration: '4 min read'
    }
  ]}
