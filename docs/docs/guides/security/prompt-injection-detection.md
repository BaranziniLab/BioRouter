Prompt injection happens when malicious instructions are hidden inside executable content. In the world of AI, prompt injection can be used to nudge AI agents (like biorouter) to run unsafe commands that compromise your environment or data.

You can help protect your biorouter workflows by enabling prompt injection detection. This feature uses pattern matching to detect common attack techniques, including:
...
When enabled, biorouter scans tool calls for risky patterns before they run:

1. **Tool call is intercepted and analyzed** - When biorouter prepares to execute a tool, the security system extracts the tool parameter text and checks it against [threat patterns](https://github.com/BaranziniLab/BioRouter/blob/main/crates/BioRouter/src/security/patterns.rs)
...
5. **You choose** whether to proceed or cancel after reviewing the alert details. Note that:
   - Each decision is logged with its finding ID in the [biorouter system logs](/docs/guides/logs#system-logs)
...
<Tabs groupId="interface">
  ...
  <TabItem value="config" label="biorouter config file">
...
Beyond prompt injection detection, biorouter automatically:
...
- [biorouter Permission Modes](/docs/guides/biorouter-permissions) - Control biorouter's autonomy level
