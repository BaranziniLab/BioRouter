...
When the main recipe is run, biorouter generates a tool for each subrecipe that:
...
**Usage:**
```bash
biorouter run --recipe code-review-pipeline.yaml --params repository_path=/path/to/repo
```
...
**Usage:**
```bash
biorouter run --recipe smart-analyzer.yaml --params repository_path=/path/to/project
```
...
**Usage:**
```bash
biorouter run --recipe travel-planner.yaml
```
