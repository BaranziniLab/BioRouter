Recipes are reusable biorouter configurations that package up instructions and settings so the setup can be easily shared and launched by others.
...
:::info
`.yml` files aren't supported by biorouter CLI.
:::
...
1. Local filesystem:
   - Current directory
   - Directories specified in [`BIOROUTER_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) environment variable
   
2. GitHub repositories:
   - Configure using [`BIOROUTER_RECIPE_GITHUB_REPO`](/docs/guides/environment-variables#recipe-configuration) configuration key
...
The `activities` field defines an optional message and clickable activity bubbles (buttons) that appears when a recipe is opened in biorouter Desktop.
...
If a recipe uses an extension that requires a secret, biorouter can prompt users to provide the secret when running the recipe:

1. When a recipe is loaded, biorouter scans all extensions (including those in subrecipes) for `env_keys` fields
...
The `required` and `optional` parameters work best for recipes opened in biorouter Desktop. If a value isn't provided for a `user_prompt` parameter, the parameter won't be substituted and may appear as literal `{{ parameter_name }}` text in the recipe output.
...
When a recipe with parameters is opened in biorouter Desktop, users are presented with a **Recipe Parameters** dialog where they can:
...
The `response` field enables recipes to enforce a final structured JSON output. When you specify a `json_schema`, biorouter will:
...
This feature is designed for **non-interactive automation** to ensure consistent, parseable output. Recipes can produce structured output when run from either the biorouter CLI or biorouter Desktop. See [use cases and ideas for automation workflows](/docs/guides/recipes/session-recipes#structured-output-for-automation).
...
- `BIOROUTER_RECIPE_RETRY_TIMEOUT_SECONDS`: Global timeout for success check commands
- `BIOROUTER_RECIPE_ON_FAILURE_TIMEOUT_SECONDS`: Global timeout for on_failure commands
...
:::note
Settings specified in a recipe will override your default biorouter configuration when that recipe is executed. If no settings are specified, biorouter will use your configured defaults.
:::
...
Recipes saved from biorouter Desktop include additional metadata fields. These fields are used by the Desktop app for organization and management but are ignored by CLI operations. 
...
Validation rules from [`validate_recipe.rs`](https://github.com/BaranziniLab/BioRouter/blob/main/crates/BioRouter/src/recipe/validate_recipe.rs) are enforced when loading recipes and used by the [`biorouter recipe validate`](/docs/guides/biorouter-cli-commands#recipe) subcommand:
