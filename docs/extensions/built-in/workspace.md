...
The one boundary is privacy: a chat on a public model cannot inject into a conversation marked private, and a chat on your institution's own model injecting into a public one asks you first, showing the exact text it would send. The tool's instructions also tell the agent to use it only when it genuinely needs to — you may be reading the conversation it interrupts.
