...
| No app-specific recipe for chat apps → wrong flow (search vs. composer vs. quick-switcher) | **P** | Added a **"Driving messaging & chat apps"** section to the instructions: composer is the box at the bottom (click → type → Return → verify); switch channels with the quick switcher (Cmd/Ctrl+K) **not** Search (Cmd/Ctrl+G); don't assume `#general` exists. |
