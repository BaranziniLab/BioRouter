...
| [Streaming and tool-call parity](streaming-and-tool-call-parity.md) | **Status: Proposed.** The root-cause analysis of why both providers are blocking today — no `stream()` override, and a tool bridge that executes silently — and the phased plan to stream text and thinking live and mirror the child's tool calls into the transcript with full card parity: each call visible as it is made, success or failure shown, arguments and result expandable. |
