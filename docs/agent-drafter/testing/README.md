...
| [Agent Drafter 100-app test-drive runbook](app-test-drive-runbook.md) | The operational runbook for driving Agent Drafter across that corpus: environment setup, the per-app authoring loop, the rubrics, and the findings log. **Superseded in part** — written 2026-07-12 against a `feat/apps-sdk-v2` git worktree that no longer exists, so the worktree requirement in its "Where the code lives" section is void; the SDK v2 primitives now sit in the main tree and everything else runs from an ordinary checkout. |
