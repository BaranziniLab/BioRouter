...
biorouter's capabilities are extensible. As an [MCP](https://modelcontextprotocol.io/) client, biorouter can connect to your apps and services through [extensions](/extensions), allowing it to work across your entire workflow.
...
> **Tip:** Consider enabling [Code Mode](/docs/guides/managing-tools/code-mode), an alternative approach to tool calling that discovers tools on demand.

### Teach biorouter your preferences
Help biorouter remember how you like to work by using [`.biorouterhints` or other context files](/docs/guides/context-engineering/using-biorouterhints) or [skills](/docs/guides/context-engineering/using-skills) for permanent project preferences and the [Memory extension](/docs/mcp/memory-mcp) for things you want biorouter to dynamically recall later. Both can help save valuable context window space while keeping your preferences available.

### Protect sensitive files
biorouter is often eager to make changes. You can stop it from changing specific files by creating a [.biorouterignore](/docs/guides/using-biorouterignore) file. In this file, you can list all the file paths you want it to avoid.
...
Administrators can use an [allowlist](/docs/guides/allowlist) to restrict biorouter to approved extensions only. This helps prevent risky installs from unknown MCP servers.

### Set up starter templates
You can turn a successful session into a reusable "[workflow](/docs/guides/workflows/session-workflows)" to share with others or use again later—no need to start from scratch.
...
Regularly [update](/docs/guides/updating-biorouter) biorouter to benefit from the latest features, bug fixes, and performance improvements.

### Pair Two Models to Save Money 
Use [lead/worker model](/docs/tutorials/lead-worker/) to have biorouter use a "lead" model for early planning before handing the task to a lower-cost "worker" model for execution.  

### Make Workflows Safe to Re-run
Write [workflows](/docs/guides/workflows/session-workflows) that check your current state before acting, so they can be run multiple times without causing any errors or duplication. 
