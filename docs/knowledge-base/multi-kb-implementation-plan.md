...
**Decision.** `<knowledge-root>/.active-kb` and `<knowledge-root>/.active-kb-sessions/<sha256(session_id)>` keep holding **one bare kb id**, exactly as today. Today's `active_kb` value *is* the primary — reading it is the whole migration. There is no runner, no eager rewrite, no first-run race, and nothing to un-migrate. Only the *names in Rust* change (`get_active_persisted` → `get_primary_persisted`, `paths::active_kb_path` → `paths::primary_kb_path`); the path helpers keep returning `.active-kb` with a doc comment saying why.
...
The body carries **three mutually exclusive** primary gestures, one per state of the stored tri-state (D5): `primary_kb` pins a base, `clear_primary` says this scope has no primary, and `inherit_primary` drops the scope's own override so it follows the machine-wide pointer again. The third is not optional decoration — `clear_primary` writes a *durable* override, and `delete_base` installs the same override in every chat that had pinned the deleted base, so without a way back over the wire such a chat could never follow the machine default again. (At machine scope there is nothing above to inherit, so `inherit_primary` and `clear_primary` coincide, exactly as the service's `PrimaryUpdate` documents.) Two gestures in one body is a **400 naming both fields**, not a precedence rule: they are three incompatible outcomes, and silently honouring one hands the caller a 200 for something it did not ask for and cannot detect. Two *spellings of the same* gesture — `clear_primary` alongside `primary_kb: null`, which is how a bundle predating `clear_primary` clears — is not a conflict. The CLI mirrors all three as `knowledge active --set` / `--clear` / `--inherit`, with `--inherit` requiring `--session <id>` because it only means something at session scope.
...
**Amendment (`--session`/`--inherit`).** One gesture does have to reach into a chat: lifting the explicit "no primary" override (D10). It only exists at session scope, `delete_base` installs it in every chat that had pinned the deleted base, and it survives every other gesture — so with a machine-only CLI the escape hatch existed in the service and was reachable from nowhere. `knowledge active` therefore takes an optional `--session <id>` that re-scopes show/`--set`/`--clear`/`--inherit` onto one chat, and `--inherit` *requires* it: at machine scope there is nothing above to inherit, so asking for it there is a clean error pointing at `--clear` rather than a silent alias. No other knowledge subcommand grows a session flag — the CLI still has no session of its own.
