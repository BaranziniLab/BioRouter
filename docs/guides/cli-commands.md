...
> **Info:** This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `biorouter acp` process. See [Using biorouter in ACP Clients](/docs/guides/acp-clients) for details.
...
/clear
```
You can also create [custom slash commands for running workflows](/docs/guides/context-engineering/slash-commands) in biorouter Desktop or the CLI. 
...
> **Info:** Syntax highlighting styles only affect the font, not the overall terminal interface. The `light` and `dark` themes have subtle differences in font color and weight.
