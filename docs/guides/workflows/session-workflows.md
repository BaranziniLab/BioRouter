Sometimes you finish a task in biorouter and realize, "Hey, this setup could be useful again." Maybe you have curated a great combination of tools, defined a clear goal, and want to preserve that flow. Or maybe you're trying to help someone else replicate what you just did without walking them through it step by step. 

You can turn your current biorouter session into a reusable workflow that includes the tools, goals, and setup you're using right now and package it into a new Agent that others (or future you) can launch with a single click.
...
## Create Workflow
  ...
  Create a workflow from the current session or from a template.
      ...
      1. While in the session you want to save as a workflow, click the  button at the bottom of the app
...
> **Warning:** You cannot create a workflow from an existing workflow session, but you can view or [edit the workflow](#edit-workflow).

  

  
   Workflow files can be either JSON (.json) or YAML (.yaml) files. While in a [session](/docs/guides/sessions/session-management#start-session), run this command to generate a workflow.yaml file in your current directory:
...
```yaml
   ...
   ```
    ...
    For detailed descriptions and example configurations of all workflow fields, see the [Workflow Reference Guide](/docs/guides/workflows/reference).

   
> **Warning:** You cannot create a workflow from an existing workflow session - the `/workflow` command will not work.

   
> **Tip:** You should [validate your workflow](#validate-workflow) to verify that it's complete and properly formatted.
...
```yaml
   ...
   See the [Workflow Reference Guide](/docs/guides/workflows/reference) for more information about workflow fields. 
...
## Edit Workflow
   ...
   1. Click the  button in the top-left to open the sidebar
   2. Click `Workflows` in the sidebar
   3. Find the workflow you want to edit and click the  button
...
> **Tip:** You can also access the edit dialog while using a workflow in a session: Just click the  button at the bottom of the app. The button shows up after you've sent your first message.
...
## Use Workflow
  ...
  1. Open the workflow using a direct link or manual URL entry, or from your Workflow library:
     ...
     **Direct Link:**
         ...
         1. Click a workflow link shared with you
     ...
     **Slash Command:**
         ...
         1. Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any biorouter chat session
...
> **Info:** - Each person gets their own private session
  - No data is shared between users
  - Your session won't affect the original workflow creator's session
  ...
  - [Scheduling a workflow](#schedule-workflow)

   #### Configure Workflow Location

  Workflows can be stored locally on your device or in a GitHub repository. Configure your workflow repository using either the `biorouter configure` command or [config file](/docs/guides/config-files#global-settings).

  
> **Tip:** - Each workflow should be in its own directory
  - Directory name matches the workflow name you use in commands
  - Workflow file can be either workflow.yaml or workflow.json
   ...
   #### Run a Workflow
       ...
       **Basic Usage** - Run once and exit (see [run options](/docs/guides/biorouter-cli-commands#run-options) and [workflow commands](/docs/guides/biorouter-cli-commands#workflow) for more):
       ```sh
       # Using workflow file in current directory or [`BIOROUTER_WORKFLOW_PATH`](/docs/guides/environment-variables#workflow-configuration) directories
       ...
       **Preview Workflow** - Use the [`explain`](/docs/guides/biorouter-cli-commands#run-options) command to view details before running:
       ...
       **With Parameters** - Supply parameter values when running workflows. See the [`run` command documentation](/docs/guides/biorouter-cli-commands#run-options) for detailed examples and options.
       ...
       **Slash Command** - Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any biorouter chat session
       ...
       **Basic Usage** - Run workflows from your configured repo using the workflow name that matches its directory (see [run options](/docs/guides/biorouter-cli-commands#run-options) and [workflow commands](/docs/guides/biorouter-cli-commands#workflow) for more):
...
> **Info:** - Each person gets their own private session
  - No data is shared between users
  - Your session won't affect the original workflow creator's session
  - The CLI can prompt users for required [extension secrets](/docs/guides/workflows/reference#extension-secrets)
...
> **Info:** If you want to validate a workflow you just created, you need to [exit the session](/docs/guides/sessions/session-management#exit-session) before running the [`validate` subcommand](/docs/guides/biorouter-cli-commands#workflow).
...
> **Info:** Each recipient gets their own private session when using your shared workflow. No data is shared between users, and your original session and workflow remain unaffected.

### Share via Workflow Link
You can share a workflow with Desktop users via a workflow link.

  
    Copy the deeplink from your Workflow Library to share with others:
    1. Click the  button in the top-left to open the sidebar
    2. Click `Workflows` in the sidebar
    3. Find the workflow you want to share and click the  button to copy the link

  
  
    Generate a deeplink from your workflow file to share with others:
    ```sh
...
When someone clicks the link, it will open biorouter Desktop with your workflow configuration. They can also use your workflow link to [import a workflow](/docs/guides/workflows/storing-workflows#importing-workflows) for future use.

### Share via Workflow File
You can share a workflow with Desktop or CLI users by sending the workflow file directly.

- Desktop users can [import the workflow](/docs/guides/workflows/storing-workflows#importing-workflows) (YAML only).
- CLI users can run a YAML or JSON workflow using `biorouter run --workflow <FILE>` or open it directly in biorouter Desktop with `biorouter workflow open <FILE>`. See the [CLI Commands guide](/docs/guides/biorouter-cli-commands#workflow) for details.

## Schedule Workflow

  
Automate biorouter workflows by running them on a schedule. When creating a schedule, you'll configure:
...
**Schedule from Workflow Library:**

   1. Click the  button in the top-left to open the sidebar
   2. Click `Workflows` in the sidebar
   3. Find the workflow you want to schedule and click the  button
...
**Schedule from Scheduler View:**
   ...
   4. In the dialog that appears, configure the schedule. For **Source**, select a `.yaml` or `.yml` file or provide a [workflow link](#share-workflow).
   5. Click `Create Schedule`

**Manage Scheduled Workflows**

Your scheduled workflows are listed in the `Scheduler` page.
Click on a schedule to view details, see when it was last run, and perform actions with the scheduled workflow:
...
At the bottom of the `Schedule Details` page you can view the list of sessions created by the scheduled workflow and open or restore each session.
  ...
  See the [`schedule` command documentation](/docs/guides/biorouter-cli-commands.md#schedule) for detailed examples and options.
...
See the [Workflow Reference Guide](/docs/guides/workflows/reference#retry) for complete retry configuration options and examples.

### Structured Output for Automation

Workflows can enforce [structured JSON output](/docs/guides/workflows/reference#response), making them ideal for automation workflows that need to parse and process agent responses reliably. Key benefits include:
...
> **Info:** Structured output is supported in workflows run in both the biorouter CLI and biorouter Desktop. However, creating and editing the `json_schema` configuration must be done manually in the workflow file.
