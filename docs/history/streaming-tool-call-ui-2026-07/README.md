...
| [Subagent delegation stress results](subagent-delegation-stress-results.md) | Delegation swept the same way: one child, many children, a child that fails, a child that tries to spawn a child, cancellation mid-flight, a twelve-wide fan-out, and steering while children run. Found that every subagent tool card in the transcript read the identical "Subagent with instructions", so parallel delegations were indistinguishable; and that a child whose turn aborted came back graded `completed` with `is_error: false`. Findings are `SUB-NN`. |
