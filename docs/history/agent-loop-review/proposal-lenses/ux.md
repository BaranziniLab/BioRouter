# Improvement proposals — Usability, UX & agent ergonomics

Lens: what the user sees and controls (plan modes, progress/todo visibility,
approvals fatigue, resume/undo affordances) **and** agent ergonomics (better tool
descriptions, repo maps, verification feedback loops, done-ness signals,
self-repair).

Grounded in the internal reviews under `internal/` and the comparison chapters
under `compare/`. Each proposal cites the report that establishes the gap.
Proposals are numbered P-1..P-34, roughly grouped: user-facing controls
(P-1..P-16), then agent ergonomics (P-17..P-34). This is an exhaustive
brainstorm, not a curated shortlist — items range from quick wins to ambitious
redesigns.
...
- **Highest-value, user-visible:** P-1 (checkpoints/undo), P-13 (repo map), P-6 (revive read-only auto-approve), P-5 (scoped permissions), P-16 (single-turn lock), P-25 (Anthropic finish_reason).
- **Highest-value, agent ergonomics:** P-17 (auto post-edit diagnostics), P-19 (done-ness gate in chat), P-21 (oversized-output preview), P-28 (unified auto-memory).
- **Quick wins (S):** P-3, P-9, P-10, P-22, P-24, P-25, P-30.
