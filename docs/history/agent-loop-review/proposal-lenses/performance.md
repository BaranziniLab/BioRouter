# Improvement proposals — Performance and efficiency

> **What this is.** The performance lens of the BioRouter agentic-loop improvement brainstorm of
> 2026-07-12: 48 proposals on latency, token efficiency, prompt caching, parallel tool execution,
> compaction cost, cheaper-model delegation, streaming, startup time and redundant-context
> elimination.
...
> merged programme was then implemented. Flagship items from this lens shipped as **BR-52** (carry
> the agent's `TokenState` in the SSE event), **BR-54** (`SharedMcpPool`, `agents/mcp_pool.rs`),
> **BR-12** (background compaction) and **BR-3** (per-model prompt variants). Treat this file as the
> record of the reasoning, not as an open work queue.
...
> restarts its numbering at `P-1`, so `P-3` here is a different proposal from `P-3` in the
> robustness or UX lens; the master list disambiguates them as `performance P-3`. `BR-NN` numbers
> are the merged master-list ids, indexed in
> [Master improvement proposals](../improvement-proposals.md).

This is one of three lens files that read the same evidence base through one concern each — this
one through performance. It is an exhaustive list, running from quick wins to ambitious redesigns,
...
and cites the review that establishes the gap. Effort is graded S (hours) / M (days) / L (weeks).
...
| `docs/performance-review-2026-06-22.md` | [Performance review findings](../../performance-2026-06/review-findings.md) |
| `docs/jcode-comparison-perf-analysis.md` | [jcode comparison analysis](../../performance-2026-06/jcode-comparison-analysis.md) |

> **What jcode is.** Several proposals cite "jcode" as an inspiration. jcode is an unrelated
> third-party Rust coding-agent harness ([`1jehuang/jcode`](https://github.com/1jehuang/jcode)) that
> was cloned and studied for the companion
...
- [Prompt caching and token efficiency](#prompt-caching-and-token-efficiency) — P-1 … P-8
- [Compaction efficiency](#compaction-efficiency) — P-9 … P-12
- [Streaming pipeline latency](#streaming-pipeline-latency) — P-13 … P-21
- [Startup time](#startup-time) — P-22 … P-25
- [Resource sharing and redundant context elimination](#resource-sharing-and-redundant-context-elimination) — P-26 … P-31
- [Parallel tool execution and tool-loop efficiency](#parallel-tool-execution-and-tool-loop-efficiency) — P-32 … P-36
- [Cheaper-model delegation](#cheaper-model-delegation) — P-37 … P-40
- [Redundant refetch and recompute elimination](#redundant-refetch-and-recompute-elimination) — P-41 … P-44
- [Server-flow efficiency and correctness-adjacent fixes](#server-flow-efficiency-and-correctness-adjacent-fixes) — P-45 … P-48
- [Summary of highest-leverage items](#summary-of-highest-leverage-items)
...
tagged the overlap. These are this file's cross-lens duplicates.

| This file | Also raised as |
|---|---|
| P-4, P-5 (dedup MOIM, refresh the clock) | `ux P-30` |
| P-7 (token-aware large-response handling) | `ux P-21`, `robustness P-42` |
| P-10 (progressive context-overflow fallback) | `robustness P-45` |
| P-32 (bound tool parallelism) | `robustness P-46` |
| P-34 (loop-level retry for streaming errors) | `robustness P-11`, `ux P-32` |
| P-38 (per-model prompt variants) | `ux P-20` |
| P-39 (read-only permission classification) | `robustness P-19`, `ux P-6` |
| P-40 (async subagent handles) | `ux P-29` |
| P-45 (per-session turn lock) | `robustness P-33`, `ux P-16` |
| P-46 (idempotency / resume token on `/reply`) | `robustness P-38` |
| P-47 (TTL on pending confirmations) | `robustness P-34`, `ux P-12` |
| P-48 (reap orphans, reconcile stuck jobs) | `robustness P-14`, `robustness P-15` |
...
## Resource sharing and redundant context elimination
...
## Parallel tool execution and tool-loop efficiency
...
## Redundant refetch and recompute elimination
...
## Server-flow efficiency and correctness-adjacent fixes
...
- [Improvement proposals — Robustness and safety](robustness.md) — the sibling lens over the same evidence, covering loop detection, checkpoints and guardrails.
- [Improvement proposals — Usability, UX and agent ergonomics](ux.md) — the sibling lens covering user-facing controls and agent ergonomics.
- [jcode comparison analysis](../../performance-2026-06/jcode-comparison-analysis.md) — the measured comparison that supplies most of the "inspired by jcode" citations here.
- [Performance review findings](../../performance-2026-06/review-findings.md) — the in-repo profiling study whose themes and A-numbers are cited throughout.
