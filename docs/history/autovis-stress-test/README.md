...
Each request drives Biorouter (dev app, GPT-5.5) in a **separate chat** to produce **one
combined artifact** (`render_dashboard`) of **2–3 figures that tell a cohesive story**.

For every run we record: did it produce one dashboard artifact, panel count, per-figure
render success (canvas/svg present, no error card), whether the report has a title +
summary + per-figure captions, and process notes (hiccups, trial-and-error, back-and-forth,
inconsistencies, inefficiencies, vulnerabilities, issues). Results log:
[`results.md`](results.md). Fixes/hardening between batches: [`hardening.md`](hardening.md).

**Pass criteria (per viz):** exactly one `ui://dashboard/*` artifact opens in the side
panel; it contains the expected number of panels; every panel renders a real figure
(a `<canvas>` or `<svg>`, or an SVG diagram) with **zero** `.panel-error` cards; the report
carries a title, a summary, and a caption under each figure. The continuing prompt then
mutates it and must re-render cleanly.

Tool coverage target: all 33 figure tools + `render_dashboard` exercised ≥2× across the 100.
...
- _Diversity note: was histogram+boxplot; swapped the boxplot for KM so `render_kaplan_meier` is exercised ≥2× (with #11)._
...
- _Diversity note: was volcano+boxplot; swapped the boxplot for a Manhattan so `render_manhattan` is exercised ≥2× (with #2)._
...
- _Diversity note: was bar+treemap; swapped the treemap for a dendrogram so `render_dendrogram` is exercised ≥2× (with #8); treemap is still covered 5×._
...
- _Diversity note: was treemap+line; swapped the line for a wordcloud so `render_wordcloud` is exercised ≥2× (with #73)._
...
- _Diversity note: was sequence+call-bar; swapped the bar for an ER diagram so `render_er_diagram` is exercised ≥2× (with #85)._
...
- _Diversity note: was flowchart+map; swapped the flowchart for a sequence diagram so `render_sequence` is exercised ≥2× (with #84); flowchart is still covered 3×._
