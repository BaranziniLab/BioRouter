# Spec 019 — Circuit Bench

> **What this is.** The static-audit record for `spec-019-circuit-bench`, the Circuit Bench app
> Agent Drafter authored during the 100-app test drive. Only the manifest and region cross-check
> ran; every browser-driven rubric check is unverified.
...
> [remediation-results.md](../remediation-results.md), so the pending checks below will never be
> filled in. This is a record of an unverified app, not work in flight. The ledger carries no
> per-app timestamp, and the run's only dated event is the 2026-07-12 provider-outage resolution
> recorded in [azure-403-outage-incident.md](../azure-403-outage-incident.md).
> **Audience:** maintainers of Agent Drafter and the Apps SDK.

`spec-019` is the nineteenth brief in the 100-idea corpus at
[hundred-app-test-specs.md](../../../agent-drafter/testing/hundred-app-test-specs.md#19-circuit-bench);
the app id Agent Drafter was given is `spec-019-circuit-bench`.
...
| Multi-agent ran (5.7) | pending | Profiles declared: designer, spice_analyst, tolerance_critic |
...
- [Spec 018 — SystemDynamics Forge](spec-018-systemdynamics-forge.md) — the last browser-verified app
  in the corpus, and the kind of evidence this record is missing.
- [Spec 020 — Diffusion Delta](spec-020-diffusion-delta.md) — the other static-only record with no
