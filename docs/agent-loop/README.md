...
| [Conversation writeback freshness](conversation-writeback-freshness.md) | Why every whole-history rewrite of a session goes through a compare-and-swap freshness guard, what the three compaction sites do when the swap is declined, the three invariants a reviewer must check, and the two writeback paths that remain unguarded. Current for the compaction sites; see also the preservation marker, which is the other half of the same guarantee. |
