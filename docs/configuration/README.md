...
This folder documents how you tell biorouter what to do before it starts a chat: which provider and model to use, how much autonomy the agent has, where it looks for extensions and workflows, and how it reports telemetry. The same settings surface in two forms — persistent YAML files in your user config directory, and environment variables that override them for a single invocation — and this folder holds the complete reference for both.
