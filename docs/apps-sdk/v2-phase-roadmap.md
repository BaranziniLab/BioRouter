...
## Phase 6 — Hardening, autorun, export v2, error loop, evals, docs (~2–3 weeks + ongoing)
...
8. **Standalone export v2 (design §3.9).**
   - **Export dialog + modes.** The panel's Export button becomes a dialog; `export_app` gains `mode: "launcher" | "full"`, `include: {knowledge_bases?, skills?, extensions?}`, `bundle_daemon: "none" | "current" | "all"`. **Launcher mode = today's scaffold, kept as a first-class choice** (app + scripts, no payload); full mode shows per-item toggles pre-checked from the app's agent config, with per-item size estimates (raw KB sources separately toggleable).
   - **Payload staging** into `payload/`: KBs via the existing `.brkb` knowledge export (`/knowledge/*` routes); skills as marketplace-format zips; external extensions as locally-installed `.brxt` bundles or pinned+checksummed BAAM registry references (builtins need nothing — they travel with the daemon). Everything enumerated in `export.json` (items, versions, checksums, required credential keys from the extensions' declared env keys, runtime prerequisites).
   - **First-run installer + consent.** `biorouter-launch.sh`/`serve.mjs` detect an uninstalled payload and drive one flow: single consent screen (§3.7 capability re-consent merged with the payload install list) → import `.brkb` KBs under their original ids, install skills/extensions → **credential setup dialog** in the SDK for unmet `export.json` credential keys (and provider setup when none configured), stored via a daemon route into the OS credential store — secrets never exist in the folder → app opens. Re-launches skip satisfied steps; same-machine self-export skips consent for grants/payload already present.
   - **OS-agnostic launchers:** add `run.ps1`/`run.bat` (Windows) beside `run.command`/`run.sh`; thin mode auto-installs the platform-matching daemon from the pinned GitHub release per-OS; fat mode stages `current` or `all` platform binaries. Export README documents macOS quarantine (right-click-Open).
   ...
   - **Graceful degradation:** missing KB grants (user unchecked the toggle) feature-detect via `has()` (harness fixture); unmet runtime prerequisites (e.g. Node for a `.brxt`) produce a clear remedy message, not a crash.
   ...
   - Tests/smoke: full export with a KB + skill + credential-requiring extension installs into a clean `XDG_CONFIG_HOME` and the app answers a KB-grounded question after the credential dialog; launcher-mode export still runs against a pre-configured store; Linux fat export boots in a clean container (mirrors the `cli-linux` release smoke); Windows launcher smoke in the release Docker path; macOS manual checklist.
...
- **Estimate:** ~12–16 weeks of focused work end-to-end (Phase 4b adds ~1–2; full-payload export adds ~1 to Phase 6), but Phase 1+2 (≈4 weeks) already dissolve the chatbot ceiling, and with the early `br.kb` read slice the KB-explorer class of apps lands by ~week 6. Multi-agent apps (4b) land by ~week 8–9. Launcher-mode export works throughout (it's the v1 scaffold + per-phase parity gate); full-payload export lands with Phase 6.
