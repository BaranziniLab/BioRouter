# Computer Controller — Comprehensive Test Plan & Findings
...
All live testing uses **Xiaomi MiMo `mimo-v2.5-pro`** (per project requirement). MiMo
is now declared vision-capable (`with_vision()`), so screenshots reach the model the
same way they do for Anthropic/OpenAI.

Legend for **Fix category**:
- **P** = prompt / context-engineering fix (tool descriptions + extension instructions)
- **T** = tool-hardening fix (Rust code change)
- **E** = environment / out-of-scope (OS permissions, hardware, model capability)

Legend for **Status**: ☐ not run · ✅ pass · ⚠️ partial · ❌ fail (see notes)

---

## Known problems this plan targets (from the user)
...
## A. Screenshots & vision (screen_capture / list_windows)

| ID | Task | Tool(s) | Expected | Targets | Fix | Status |
|----|------|---------|----------|---------|-----|--------|
...
## E. File operations & document tools
...
## H. Error / permission / robustness

| ID | Task | Expected | Targets | Fix | Status |
|----|------|----------|---------|-----|--------|
...
See `docs/computer-controller-test-results.md` for the executed-run notes (live MiMo
TUI/GUI runs + automated tests). Cases that require specific third-party apps (Slack,
Notes) or OS permissions that are not granted in this environment are marked **E** and
verified by code review + the honest-error path rather than a live click.
