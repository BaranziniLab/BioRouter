...
vi.mock('./components/settings/chat/BioRouterHintsModal', () => ({
  BioRouterHintsModal: () => null,
}));
...
const mockAppConfig = {
  get: vi.fn((key: string) => {
    if (key === 'BIOROUTER_WORKING_DIR') return '/test/dir';
    return null;
  }),
};
...
describe('App Component - Brand New State', () => {
  ...
  it('should redirect to "/" when app is brand new (no provider configured)', async () => {
    ...
    mockElectron.getConfig.mockReturnValue({
      BIOROUTER_DEFAULT_PROVIDER: null,
      BIOROUTER_DEFAULT_MODEL: null,
      BIOROUTER_ALLOWLIST_WARNING: false,
    });
