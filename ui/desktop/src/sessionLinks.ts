 ...
 * @param url The deep link URL (biorouter://sessions/:shareToken)
...
export async function openSharedSessionFromDeepLink(
  ...
  try {
    if (!url.startsWith('biorouter://sessions/')) {
      throw new Error('Invalid URL: URL must use the biorouter://sessions/ scheme');
    }

    // Extract the share token from the URL
    const shareToken: string = url.replace('biorouter://sessions/', '');
    ...
    setView('sharedSession', {
      ...
      shareToken: url.replace('biorouter://sessions/', ''),
      baseUrl,
    });
