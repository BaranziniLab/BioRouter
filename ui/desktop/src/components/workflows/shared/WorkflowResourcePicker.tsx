    ...
    <div className="space-y-2">
      <div className="flex items-start justify-between gap-3">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            <div className="flex h-8 items-center gap-2 rounded-md bg-background-default px-2 ring-1 ring-border-input">
            ...
            <div className="mt-2 max-h-72 overflow-y-auto">
              {filteredItems.length === 0 ? (
                <div className="px-2 py-6 text-center text-xs text-text-muted">{emptyText}</div>
              ) : (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? 'bg-background-medium hover:bg-background-muted'
                          : 'hover:bg-background-medium/70'
                      )}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          {item.badge && (
                            <span className="shrink-0 rounded bg-background-muted px-1.5 py-0.5 text-[11px] uppercase text-text-muted">
                              {item.badge}
                            </span>
                          )}
                        </div>
                        {item.description && (
                          <p className="truncate text-[11px] text-text-muted">{item.description}</p>
                        )}
                      </div>
                      {onDefaultIdChange && isSelected && (
                        <button
                          type="button"
                          className={cn(
                            'inline-flex h-6 shrink-0 items-center gap-1 rounded px-1.5 text-[11px] transition-colors',
                            isDefault
                              ? 'bg-background-default text-text-default'
                              : 'text-text-muted hover:bg-background-default hover:text-text-default'
                          )}
                          onClick={(event) => {
