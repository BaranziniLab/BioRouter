...
import { WorkflowFormFields, extractTemplateVariables } from '../WorkflowFormFields';
import { type WorkflowFormData } from '../workflowFormSchema';
...
describe('WorkflowFormFields', () => {
  const useTestForm = (initialValues?: Partial<WorkflowFormData>) => {
  ...
  } & Omit<Parameters<typeof WorkflowFormFields>[0], 'form'>) => {
    const form = useTestForm(initialValues);

    return <WorkflowFormFields form={form} {...props} />;
  };
  ...
  describe('Form Interactions', () => {
    it('allows typing in text fields', async () => {
      ...
      await user.type(descriptionInput, 'A test workflow');
      expect(descriptionInput).toHaveValue('A test workflow');
    });
  ...
  describe('Pre-filled Values', () => {
    it('displays pre-filled form values', () => {
      const initialValues: Partial<WorkflowFormData> = {
  ...
  describe('Parameter Auto-Detection', () => {
    ...
    it('handles activities field for parameter detection', async () => {
      ...
      await user.paste('Welcome to {{workflow_name}}!');
      expect(messageInput).toHaveValue('Welcome to {{workflow_name}}!');
    });
    ...
    it('actually detects and creates parameters from template variables', async () => {
      ...
      const TestComponent = () => {
        ...
        return <WorkflowFormFields form={form} />;
      };
