...
import { WorkflowFormData, workflowFormSchema } from '../workflowFormSchema';

describe('workflowFormSchema', () => {
  const validFormData: WorkflowFormData = {
  ...
  describe('Zod Schema Validation', () => {
    describe('Basic Validation', () => {
      it('validates a complete valid form', () => {
        const result = workflowFormSchema.safeParse(validFormData);
    ...
    describe('Edge Cases', () => {
      it('handles null values gracefully', () => {
        ...
        const result = workflowFormSchema.safeParse(dataWithNulls);
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.issues.some((issue) => issue.path.includes('title'))).toBe(true);
          expect(result.error.issues.some((issue) => issue.path.includes('description'))).toBe(
      ...
      it('handles undefined values gracefully', () => {
        ...
        const result = workflowFormSchema.safeParse(dataWithUndefined);
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.issues.some((issue) => issue.path.includes('title'))).toBe(true);
          expect(result.error.issues.some((issue) => issue.path.includes('description'))).toBe(
      ...
      it('handles completely empty form data', () => {
        ...
        const result = workflowFormSchema.safeParse(emptyData);
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.issues.some((issue) => issue.path.includes('title'))).toBe(true);
          expect(result.error.issues.some((issue) => issue.path.includes('description'))).toBe(
      ...
      it('handles minimal valid form data', () => {
        ...
        const result = workflowFormSchema.safeParse(minimalData);
        expect(result.success).toBe(true);
      });
    });
  });
