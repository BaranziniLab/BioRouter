...
function userFacingMessage(error: ChatTurnErrorData): string {
  const providerMessage = error.message.match(/"message":\s*String\("((?:\\.|[^"\\])*)"\)/)?.[1];
  if (providerMessage) return decodeProviderMessage(providerMessage);
