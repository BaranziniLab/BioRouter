...
test.describe('Context Management E2E Tests', () => {
  ...
  test('should perform manual compaction when compact button is clicked', async () => {
    ...
    // Verify compaction loading state. The composer's `data-working="true"`
    // attribute carries no text, so "compacting" specifically can no longer be
    // told apart from any other busy state at the composer -- assert against
    // the transcript's turn activity indicator instead, which sets
    // data-phase="compacting" while a compaction turn is in flight.
  ...
  test('should maintain conversation flow after compaction', async () => {
    ...
    await page.waitForSelector(TURN_RUNNING, { timeout: 30000 });
  ...
  test('should show appropriate loading states during compaction', async () => {
    ...
    // Verify loading state immediately after clicking compact. Same reasoning
    // as above: the composer carries no text, so check the transcript's
    // compacting-phase indicator instead.
    const compactingIndicator = page.locator(
      '[data-testid="turn-activity-indicator"][data-phase="compacting"]'
    );
    await expect(compactingIndicator).toBeVisible();
