# Improvement proposals — Robustness & safety

Lens: loop/stuck detection, error-streak handling, checkpoints/undo of agent
edits, crash/restart survival, permission & guardrail gaps, hook event coverage,
sandbox, and conversation-invariant edge cases (compaction boundaries,
cancellation, tool_call/result pairing).

Grounded in the internal reviews under `internal/` (core-loop, loop-detection,
hooks, guardrails-permissions, compaction, long-running, state-awareness,
verification, server-flow, context-injection) and the `compare/` chapters
(safety, context). Ordered roughly by theme, not priority. Each proposal is
self-contained; several are cheap "quick wins" and a few are ambitious
redesigns.
